...
function c101010204.spfilter(c)
	return c:IsCode(101010152) or c:IsCode(101010153)
end
function c101010204.fscondition(e,g,gc,chkf)
	if g==nil then return false end
	if gc then
		local mg=g:Filter(Card.IsFusionSetCard,gc,0x167)
		return (gc:IsFusionSetCard(0x167) and mg:IsExists(c101010204.spfilter,1,nil)) or (c101010204.spfilter(gc) and mg:GetCount()>0)
	end
	local fs=false
	local mg=g:Filter(Card.IsFusionSetCard,nil,0x167)
	if mg:IsExists(aux.FConditionCheckF,1,nil,chkf) then fs=true end
	return mg:IsExists(c101010204.spfilter,1,nil) and mg:GetCount()>0 and (fs or chkf==PLAYER_NONE)
end
function c101010204.fsoperation(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if gc then
		local sg=eg:Filter(Card.IsFusionSetCard,gc,0x167)
		local g=Group.FromCards(gc)
		while sg:IsExists(Card.IsFusionSetCard,gc,0x167) do
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			g:Merge(sg:Select(tp,1,1,nil))
			sg:Remove(Card.IsCode,nil,g:GetFirst():GetCode())
			if sg:FilterCount(Card.IsFusionSetCard,gc,0x167)==0 or not Duel.SelectYesNo(tp,aux.Stringid(101010038,0)) then break end
		end
		Duel.SetFusionMaterial(g)
		return
	end
	local sg1=eg:Filter(c101010204.spfilter,nil)
	local g1=nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	while sg2:IsExists(Card.IsFusionSetCard,1,nil,0x167) do
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
		g2:Merge(sg2:Select(tp,1,1,nil))
		sg2:Remove(Card.IsCode,nil,g2:GetFirst():GetCode())
		if sg2:FilterCount(Card.IsFusionSetCard,nil,0x167)==0 or not Duel.SelectYesNo(tp,aux.Stringid(101010038,0)) then break end
	end
	g1:Merge(g2)
