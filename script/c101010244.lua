...
--change name
	...
	e0:SetCondition(c101010244.rtcon)
	...
	e3:SetOperation(c101010244.matop)
	c:RegisterEffect(e3)
end
function c101010244.ffilter(c)
	return (c:IsType(TYPE_MONSTER) or (c:IsFaceup() and c:IsOnField())) and c:IsAbleToGraveAsCost()
end
function c101010244.spscon(e,c)
	if c==nil then return true end
	local mg=Duel.GetMatchingGroup(c101010244.ffilter,tp,LOCATION_MZONE+LOCATION_HAND,0,nil)
	return mg:CheckWithSumEqual(Card.GetLevel,c:GetOriginalLevel(),1,99,nil)
end
function c101010244.spsop(e,tp,eg,ep,ev,re,r,rp,c)
...
function c101010244.rtcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsRelateToBattle()
end
function c101010244.filter(c,e)
	return c:IsSetCard(0x5cd) and c:IsAbleToHand()
end
function c101010244.rttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c101010244.rtop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c101010244.filter,tp,LOCATION_DECK,0,1,1,nil)
	if g:GetCount()>0 then
		Duel.SendtoHand(g,nil,REASON_EFFECT)
		Duel.ConfirmCards(1-tp,g)
	end
end
function c101010244.matop(e,tp,eg,ep,ev,re,r,rp)
	...
	e1:SetValue(c101010244.aclimit)
	e1:SetCondition(c101010244.actcon)
	e1:SetReset(RESET_EVENT+0x1fe0000)
	rc:RegisterEffect(e1)
end
function c101010244.aclimit(e,re,tp)
	return not re:GetHandler():IsImmuneToEffect(e)
end
function c101010244.actcon(e)
	return (Duel.GetAttacker()==e:GetHandler() or Duel.GetAttackTarget()==e:GetHandler()) and e:GetHandler():GetBattleTarget()~=nil
end
function c101010244.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010244.spop(e,tp,eg,ep,ev,re,r,rp)
