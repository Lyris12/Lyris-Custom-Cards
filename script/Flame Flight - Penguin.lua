...
function c101010558.initial_effect(c)
	--spsummon condition
	...
	e0:SetCountLimit(1)
	e0:SetCondition(c101010558.con)
	e0:SetTarget(c101010558.target)
	e0:SetOperation(c101010558.op)
	c:RegisterEffect(e0)
	--fusion summon
	c:EnableReviveLimit()
	aux.AddFusionProcFun2(c,c101010558.ffilter1,c101010558.ffilter2,true)
end
function c101010558.ffilter1(c)
	return c:IsAttribute(ATTRIBUTE_WATER) or (c:IsHasEffect(101010560) and not c:IsLocation(LOCATION_DECK))
end
function c101010558.ffilter2(c)
	return c:IsRace(RACE_AQUA+RACE_WINDBEAST) or (c:IsHasEffect(101010576) and not c:IsLocation(LOCATION_DECK))
end
function c101010558.filter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and (c:IsType(TYPE_FUSION))
end
function c101010558.con(e,tp,eg,ep,ev,re,r,rp)
	...
	return not e:GetHandler():IsStatus(STATUS_BATTLE_DESTROYED) and tg and tg:IsExists(c101010558.filter,1,nil) and Duel.IsChainNegatable(ev)
end
function c101010558.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_NEGATE,eg,1,0,0)
end
function c101010558.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
