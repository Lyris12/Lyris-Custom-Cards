...
function ref.fscon(e,g,gc,chkf)
	...
	if gc then
		...
		if (gc:IsFusionCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(ref.material,1,gc) then
			return true
...
function ref.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	if gc then
		if (gc:IsFusionCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and g:IsExists(ref.material,1,gc) then
...
function ref.actcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetFlagEffect(tp,id)==0 and (not e:GetHandler():IsStatus(STATUS_SET_TURN) or e:GetHandler():GetFlagEffect(101010170)~=0)
end
