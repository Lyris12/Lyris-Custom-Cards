...
function c101010202.initial_effect(c)
	...
	e1:SetCondition(c101010202.fscon)
	e1:SetOperation(c101010202.fsop)
	c:RegisterEffect(e1)
	--destroy
	...
	e4:SetOperation(c101010202.desop2)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	e2:SetDescription(aux.Stringid(101010202,0))
	e2:SetCategory(CATEGORY_DESTROY+CATEGORY_DAMAGE+CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_ACTIVATE+EFFECT_TYPE_QUICK_O)
	e2:SetCode(EVENT_FREE_CHAIN)
	e2:SetProperty(EFFECT_FLAG_CARD_TARGET)
	e2:SetCondition(c101010202.actcon)
	e2:SetTarget(c101010202.acttg)
	e2:SetOperation(c101010202.act)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	e3:SetDescription(aux.Stringid(101010202,1))
	...
	e7:SetCondition(c101010202.con)
	e7:SetTarget(c101010202.tg)
	e7:SetOperation(c101010202.op)
	c:RegisterEffect(e7)
end
function c101010202.material(c)
	return bit.band(c:GetOriginalType(),TYPE_SPELL)==TYPE_SPELL and c:IsSetCard(0x5d)
end
function c101010202.fscon(e,g,gc,chkf)
	...
	if not gc then
		...
		local sg=Duel.GetMatchingGroup(c101010202.material,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local mg=g:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if sg then mg:Merge(sg) end
	if gc then
		--if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
		if (gc:IsFusionCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(c101010202.material,1,gc) then
			return true
		elseif c101010202.material(gc) then
			local g1=Group.CreateGroup() local g2=Group.CreateGroup()
			local tc=mg:GetFirst()
			while tc do
				if tc:IsFusionCode(76263644) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)
					then g1:AddCard(tc) end
				if c101010202.material(tc) then g2:AddCard(tc) end
				tc=mg:GetNext()
			end
			g1:RemoveCard(gc)
	...
	local tc=mg:GetFirst()
	while tc do
		local c1=tc:IsFusionCode(76263644) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)
		local c2=c101010202.material(tc)
		if c1 or c2 then
	...
	return b1+b2+bw>=1+1 and (fs or chkf==PLAYER_NONE)
end
function c101010202.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if not gc then
		local loc1=LOCATION_SZONE
		local loc2=LOCATION_SZONE
		...
		local mg=Duel.GetMatchingGroup(c101010202.material,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if mg then g:Merge(mg) end
	if gc then
		if (gc:IsFusionCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and g:IsExists(c101010202.material,1,gc) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local g1=g:FilterSelect(tp,c101010202.material,1,1,gc)
			Duel.SetFusionMaterial(g1)
		else
			local sg1=Group.CreateGroup() local sg2=Group.CreateGroup()
			local tc=g:GetFirst()
			while tc do
				if tc:IsFusionCode(76263644) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then sg1:AddCard(tc) end
				if c101010202.material(tc) then sg2:AddCard(tc) end
				tc=g:GetNext()
			end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	while tc do
		if tc:IsFusionCode(76263644) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then sg1:AddCard(tc) end
		if c101010202.material(tc) then sg2:AddCard(tc) if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
...
function c101010202.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if Duel.MoveToField(c,tp,tp,LOCATION_SZONE,POS_FACEDOWN,true) then
		Duel.RegisterFlagEffect(tp,101010202,RESET_PHASE+PHASE_END,0,1)
		return true
	else return false end
end
function c101010202.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetPreviousControler()==tp
end
function c101010202.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010202.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP_DEFENSE)
	end
end
function c101010202.actcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_SET_TURN) or e:GetHandler():GetFlagEffect(101010091)~=0
end
function c101010202.acttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010202.act(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		...
		c101010202.after(e,tp)
	end
end
function c101010202.after(e,tp)
