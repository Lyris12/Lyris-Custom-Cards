...
aux.EnablePendulumAttribute(c)
	...
	e2:SetOperation(c101010265.chop2)
	c:RegisterEffect(e2)
	--special summon
	...
	e3:SetCode(EVENT_CUSTOM+101010265)
	e3:SetRange(LOCATION_PZONE)
	e3:SetCondition(c101010265.sumcon)
	e3:SetTarget(c101010265.sumtg)
	e3:SetOperation(c101010265.sumop)
	c:RegisterEffect(e3)
...
function c101010265.chop2(e,tp,eg,ep,ev,re,r,rp)
...
function c101010265.sumop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c101010265.filter,e:GetHandler():GetControler(),LOCATION_DECK,0,nil,e,tp):RandomSelect(tp,1)
	local tc=g:GetFirst()
	if tc then
...
--[[function c101010265.thfilter(c)
	return c:IsSetCard(0x613) and not c:IsCode(101010265) and c:IsType(TYPE_PENDULUM) and c:IsAbleToHand()
end
function c101010265.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010265.thfilter,tp,LOCATION_DECK,0,1,nil) end
	Duel.SetOperationInfo(0,CATEGORY_TOHAND,nil,1,tp,LOCATION_DECK)
end
function c101010265.thop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g=Duel.SelectMatchingCard(tp,c101010265.thfilter,tp,LOCATION_DECK,0,1,1,nil)
