...
function c101010600.initial_effect(c)
	...
	ae3:SetCondition(c101010600.con)
	ae3:SetOperation(c101010600.acop)
	...
	ae0:SetOperation(c101010600.eop)
	...
	ae1:SetTarget(c101010600.reptg)
	ae1:SetOperation(c101010600.repop)
	...
	ae2:SetCountLimit(1)
	ae2:SetCondition(c101010600.con)
	ae2:SetCost(c101010600.cost)
	ae2:SetOperation(c101010600.op)
	...
	e1:SetCondition(c101010600.spcon)
	e1:SetOperation(c101010600.spop)
	e1:SetValue(8751)
	c:RegisterEffect(e1)
	...
	e2:SetOperation(c101010600.addc)
	c:RegisterEffect(e2)
	--counter (Do Not Remove)
	...
	e4:SetCost(c101010600.acos)
	e4:SetTarget(c101010600.addct2)
	e4:SetOperation(c101010600.addc2)
	c:RegisterEffect(e4)
	...
	e5:SetCondition(c101010600.con)
	e5:SetValue(c101010600.efilter)
	c:RegisterEffect(e5)
...
function c101010600.rfilter1(c)
	return c:GetLevel()==4 and c:IsRace(RACE_FAIRY)
end
function c101010600.rfilter2(c)
	return c:GetLevel()==4 and c:IsAttribute(ATTRIBUTE_WATER)
end
function c101010600.spcon(e,c)
	if c==nil then return true end
	if c:IsType(TYPE_PENDULUM) and c:IsFaceup() then return false end
	local tp=c:GetControler()
	local g1=Duel.GetMatchingGroup(c101010600.rfilter1,tp,LOCATION_MZONE,0,nil)
	if g1:GetCount()>0 then
		local g2=Duel.IsExistingMatchingCard(c101010600.rfilter2,tp,LOCATION_MZONE,0,1,g1:GetFirst(),nil)
		return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2 and g2
	end
	return false
end
function c101010600.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g2=Duel.SelectMatchingCard(tp,c101010600.rfilter1,tp,LOCATION_MZONE,0,1,1,nil)
	local g1=Duel.SelectMatchingCard(tp,c101010600.rfilter2,tp,LOCATION_MZONE,0,1,1,g2:GetFirst())
	g1:Merge(g2)
	c:SetMaterial(g1)
	Duel.SendtoGrave(g1,REASON_MATERIAL+8751)
end
function c101010600.addc(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():AddCounter(0x88,8)
end
function c101010600.cfilter(c)
	return c:IsType(TYPE_SPELL) and c:IsAbleToGraveAsCost()
end
function c101010600.acos(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010600.cfilter,tp,LOCATION_HAND,0,1,e:GetHandler()) end
	Duel.DiscardHand(tp,c101010600.cfilter,1,1,REASON_COST)
end
function c101010600.addct2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_COUNTER,nil,3,0,0x88)
end
function c101010600.addc2(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():IsRelateToEffect(e) then
		e:GetHandler():AddCounter(0x88,3)
	end
end
function c101010600.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetCounter(0x88)>2
end
function c101010600.efilter(e,te)
	return te:IsHasCategory(CATEGORY_NEGATE+CATEGORY_DISABLE)
end
function c101010600.acop(e,tp,eg,ep,ev,re,r,rp)
...
function c101010600.filter(c,e,tp)
	return c:IsSetCard(0x785e) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c101010600.op(e,tp,eg,ep,ev,re,r,rp)
	...
	if Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)~=0 and bit.band(tc:GetFirst():GetOriginalType(),TYPE_FUSION)==TYPE_FUSION and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local g=Duel.SelectMatchingCard(tp,c101010600.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil,e,tp)
