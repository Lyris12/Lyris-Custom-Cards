...
function c101010596.initial_effect(c)
	--removal
	...
	ae1:SetCountLimit(1)
	ae1:SetCost(c101010596.cost)
	ae1:SetTarget(c101010596.tg)
	ae1:SetOperation(c101010596.op)
	...
	e1:SetCondition(c101010596.spcon)
	e1:SetOperation(c101010596.spop)
...
function c101010596.spfilter(c,v,y,z)
	local atk=c:GetAttack()
	return c:GetLevel()==v and atk>=y and atk<=z and c:IsAbleToRemoveAsCost() and (c:IsSetCard(0x785e) or c:IsSetCard(0x53))
end
function c101010596.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and Duel.IsExistingMatchingCard(c101010596.spfilter,tp,LOCATION_MZONE,0,1,nil,8,2800,3600)
end
function c101010596.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.SelectMatchingCard(tp,c101010596.spfilter,tp,LOCATION_MZONE,0,1,1,nil,8,2800,3600)
...
function c101010596.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsAbleToRemoveAsCost,tp,LOCATION_HAND,0,1,nil) end
	local g=Duel.SelectMatchingCard(tp,Card.IsAbleToRemoveAsCost,tp,LOCATION_HAND,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
end
function c101010596.tfilter(c)
	return c101010596.filter(c,c:GetOriginalType())
end
function c101010596.filter(c,typ)
...
function c101010596.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingTarget(c101010596.tfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,e:GetHandler()) end
	local g=Duel.SelectTarget(tp,c101010596.tfilter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,e:GetHandler())
...
function c101010596.op(e,tp,eg,ep,ev,re,r,rp)
