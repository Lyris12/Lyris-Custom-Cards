...
function c101010079.initial_effect(c)
	...
	e0:SetOperation(c101010079.sprop)
	...
	e6:SetValue(c101010079.efilter)
	c:RegisterEffect(e6)
	...
	e4:SetLabel(0)
	e4:SetCost(c101010079.spcost)
	e4:SetTarget(c101010079.sptg)
	e4:SetOperation(c101010079.spop)
	c:RegisterEffect(e4)
	...
	e2:SetCondition(c101010079.indcon)
	e2:SetValue(1)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e5:SetCost(c101010079.tdcost)
	e5:SetTarget(c101010079.tdtg)
	e5:SetOperation(c101010079.tdop)
	c:RegisterEffect(e5)
	...
	e1:SetCost(c101010079.tdcost2)
	e1:SetTarget(c101010079.tdtg2)
	e1:SetOperation(c101010079.tdop)
	c:RegisterEffect(e1)
end
function c101010079.spfilter1(c)
	return c:IsFaceup() and c:IsType(TYPE_TUNER)  and c:IsAttribute(ATTRIBUTE_WIND) and c:IsAbleToDeck()
end
function c101010079.spfilter2(c)
	return c:IsFaceup() and c:IsCode(160000895) and c:IsAbleToDeck()
end
function c101010079.sprcon(e,c,smat,mg)
	local g=e:GetLabelObject()
	c:SetMaterial(g)
	Duel.SendtoDeck(g,nil,2,REASON_MATERIAL+REASON_SYNCHRO)
	g:DeleteGroup()
end
function c101010079.efilter(e,te)
	local c=te:GetHandler()
	return c:IsCode(503) or c:IsCode(539)
end
function c101010079.indcon(e)
	return Duel.IsExistingMatchingCard(Card.IsType,e:GetHandlerPlayer(),LOCATION_MZONE,0,1,nil,TYPE_TOKEN)
end
function c101010079.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010079.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=e:GetLabel()
...
function c101010079.tdcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010079.cfilter(c)
	return c:IsSetCard(0x1d5d) and c:IsAbleToDeckAsCost()
end
function c101010079.tdcost2(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():GetAttackAnnouncedCount()==0
	and Duel.IsExistingMatchingCard(c101010079.cfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	local g=Duel.SelectMatchingCard(tp,c101010079.cfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
...
function c101010079.tdtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and c101010079.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101010079.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TODECK)
	local g=Duel.SelectTarget(tp,c101010079.filter,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,g,1,0,0)
end
function c101010079.tdtg2(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010079.tdop(e,tp,eg,ep,ev,re,r,rp)
