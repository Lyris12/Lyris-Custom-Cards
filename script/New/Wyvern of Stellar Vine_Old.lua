...
function c101010237.initial_effect(c)
	...
	e2:SetCondition(c101010237.spcon)
	e2:SetTarget(c101010237.sptg)
	e2:SetOperation(c101010237.spop)
	c:RegisterEffect(e2)
	--
	...
	e0:SetCondition(c101010237.con)
	e0:SetTarget(c101010237.tg)
	e0:SetOperation(c101010237.op)
	c:RegisterEffect(e0)
end
function c101010237.spcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsPreviousLocation(LOCATION_DECK+LOCATION_GRAVE+LOCATION_HAND)
end
function c101010237.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,1,tp,true,true) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010237.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) and Duel.SpecialSummon(c,1,tp,tp,true,true,POS_FACEUP_ATTACK)>0 then
		c:CompleteProcedure()
	end
end
function c101010237.con(e,tp,eg,ep,ev,re,r,rp)
...
function c101010237.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_REMOVED) and chkc:IsControler(tp) and c101010237.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101010237.filter,tp,LOCATION_REMOVED,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(101010237,0))
	local g1=Duel.SelectTarget(tp,c101010237.filter,tp,LOCATION_REMOVED,0,1,2,nil)
