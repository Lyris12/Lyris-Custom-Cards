...
function c101010050.initial_effect(c)
	...
	e1:SetCondition(c101010050.fscon)
	e1:SetOperation(c101010050.fsop)
	c:RegisterEffect(e1)
	--equip
	...
	e2:SetTarget(c101010050.eqtg)
	e2:SetOperation(c101010050.eqop)
	c:RegisterEffect(e2)
	--copy
	...
	e3:SetOperation(c101010050.effop)
	c:RegisterEffect(e3)
	--Destroy replace
	...
	e4:SetTarget(c101010050.desreptg)
	e4:SetOperation(c101010050.desrepop)
	c:RegisterEffect(e4)
end
function c101010050.brfilter(c)
	return c:IsType(TYPE_SPELL) and c:IsSetCard(0x5d)
end
function c101010050.fscon(e,g,gc,chkf)
	...
	if not gc then
		...
		local sg=Duel.GetMatchingGroup(c101010050.brfilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local mg=g:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if sg then mg:Merge(sg) end
	if gc then
		if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
		if (gc:IsFusionCode(40418351) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(c101010050.brfilter,1,gc) then
			return true
			elseif c101010050.brfilter(gc) then
			...
			while tc do
				...
				if c101010050.brfilter(tc) then g2:AddCard(tc) end
				tc=mg:GetNext()
			end
			g1:RemoveCard(gc)
	...
	while tc do
		...
		local c2=c101010050.brfilter(tc)
		if c1 or c2 then
...
function c101010050.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if not gc then
		local loc1=LOCATION_SZONE
		local loc2=LOCATION_SZONE
		...
		local mg=Duel.GetMatchingGroup(c101010050.brfilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if mg then g:Merge(mg) end
	if gc then
		if (gc:IsFusionCode(40418351) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and g:IsExists(c101010050.brfilter,1,gc) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local g1=g:FilterSelect(tp,c101010050.brfilter,1,1,gc)
			...
			while tc do
				...
				if c101010050.brfilter(tc) then sg2:AddCard(tc) end
				tc=g:GetNext()
			end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	while tc do
		if tc:IsFusionCode(40418351) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then sg1:AddCard(tc) end
		if c101010050.brfilter(tc) then sg2:AddCard(tc) if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
...
function c101010050.filter(c)
	return c:IsRace(RACE_DRAGON)
end
function c101010050.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c101010050.filter(chkc) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
	and Duel.IsExistingTarget(c101010050.filter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,nil,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	local tc=Duel.SelectMatchingCard(tp,c101010050.filter,tp,LOCATION_GRAVE,LOCATION_GRAVE,1,1,nil)
	Duel.SetTargetCard(tc)
	Duel.SetOperationInfo(0,CATEGORY_LEAVE_GRAVE,tc,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,tc,1,0,0)
end
function c101010050.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
		...
		e1:SetValue(c101010050.eqlimit)
...
function c101010050.eqlimit(e,c)
	return e:GetOwner()==c
end
function c101010050.effop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local wg=Duel.GetMatchingGroup(c101010050.filter,tp,LOCATION_SZONE,0,nil)
...
function c101010050.repfilter(c)
	return c:IsLocation(LOCATION_SZONE) and not c:IsStatus(STATUS_DESTROY_CONFIRMED)
end
function c101010050.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
		local g=c:GetEquipGroup()
		return not c:IsReason(REASON_REPLACE) and g:IsExists(c101010050.repfilter,1,nil)
	end
	local g=c:GetEquipGroup()
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local sg=g:FilterSelect(tp,c101010050.repfilter,1,1,nil)
	Duel.SetTargetCard(sg)
	return true
end
function c101010050.desrepop(e,tp,eg,ep,ev,re,r,rp)
