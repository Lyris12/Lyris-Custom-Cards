...
function c101010027.initial_effect(c)
	--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcFun2(c,aux.FilterBoolFunction(c101010027.ffilter),aux.FilterBoolFunction(Card.IsAttribute,ATTRIBUTE_WATER),true)
	--spsummon condition
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_SINGLE)
	e1:SetProperty(EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE)
	e1:SetCode(EFFECT_SPSUMMON_CONDITION)
	e1:SetValue(c101010027.splimit)
	c:RegisterEffect(e1)
	--special summon rule
	...
	e2:SetCondition(c101010027.sprcon)
	e2:SetOperation(c101010027.sprop)
	c:RegisterEffect(e2)
	...
	e4:SetTarget(c101010027.eqtg)
	e4:SetOperation(c101010027.eqop)
	c:RegisterEffect(e4)
	--material check
	local e5=Effect.CreateEffect(c)
	e5:SetType(EFFECT_TYPE_SINGLE)
	e5:SetCode(EFFECT_MATERIAL_CHECK)
	e5:SetValue(c101010027.matcheck)
	e5:SetLabelObject(e4)
	c:RegisterEffect(e5)
end
function c101010027.ffilter(c)
	return c:IsSetCard(0x93) and c:IsRace(RACE_MACHINE)
end
function c101010027.splimit(e,se,sp,st)
	return e:GetHandler():GetLocation()~=LOCATION_EXTRA
end
function c101010027.spfilter1(c,tp,ft)
	if c:IsSetCard(0x93) and c:IsRace(RACE_MACHINE) and c:IsAbleToGraveAsCost() and c:IsCanBeFusionMaterial() and (c:IsControler(tp) or c:IsFaceup()) then
		if ft>0 or (c:IsControler(tp) and c:IsLocation(LOCATION_MZONE)) then
			return Duel.IsExistingMatchingCard(c101010027.spfilter2,tp,LOCATION_MZONE,LOCATION_MZONE,1,c,tp)
			else
			return Duel.IsExistingMatchingCard(c101010027.spfilter2,tp,LOCATION_MZONE,0,1,c,tp)
		end
	else return false end
end
function c101010027.spfilter2(c,tp)
...
function c101010027.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	return ft>-1 and Duel.IsExistingMatchingCard(c101010027.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil,tp,ft)
end
function c101010027.sprop(e,tp,eg,ep,ev,re,r,rp,c)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	Duel.Hint(HINT_SELECTMSG,tp,aux.Stringid(101010027,0))
	local g1=Duel.SelectMatchingCard(tp,c101010027.spfilter1,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,1,nil,tp,ft)
	local tc=g1:GetFirst()
	local g=Duel.GetMatchingGroup(c101010027.spfilter2,tp,LOCATION_MZONE,LOCATION_MZONE,nil,tp)
	local g2=nil
...
function c101010027.matcheck(e,c)
...
function c101010027.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010027.eqop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsFaceup() and c:IsRelateToEffect(e) and tc:IsRelateToEffect(e) then
		...
		e1:SetValue(c101010027.eqlimit)
		...
		if bit.band(ct,ATTRIBUTE_LIGHT)~=0 then
			...
			e2:SetCondition(c101010027.discon)
			e2:SetCost(c101010027.discost)
			e2:SetTarget(c101010027.distg)
			e2:SetOperation(c101010027.disop)
			c:RegisterEffect(e2)
		end
...
function c101010027.eqlimit(e,c)
	return e:GetOwner()==c
end
function c101010027.discon(e,tp,eg,ep,ev,re,r,rp)
	if not re:IsHasProperty(EFFECT_FLAG_CARD_TARGET) then return false end
	local tg=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
	return tg and tg:IsContains(e:GetHandler()) and Duel.IsChainNegatable(ev)
end
function c101010027.discost(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then
...
function c101010027.distg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010027.disop(e,tp,eg,ep,ev,re,r,rp)
