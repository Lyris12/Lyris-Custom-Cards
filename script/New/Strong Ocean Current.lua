...
function c101010224.initial_effect(c)
	...
	e0:SetCountLimit(1,101010224)
	e0:SetOperation(c101010224.op)
	c:RegisterEffect(e0)
	...
	e1:SetOperation(c101010224.drop)
	c:RegisterEffect(e1)
end
c101010224.sea_scout_list=true
function c101010224.cfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x5cd)
end
function c101010224.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsRelateToEffect(e) then return end
	local g=Duel.GetMatchingGroup(c101010224.cfilter,tp,LOCATION_MZONE,0,nil)
...
function c101010224.filter(c,eg)
	if eg:GetCount()~=1 then return false end
	local tc=eg:GetFirst()
	return tc:IsAttribute(ATTRIBUTE_WATER) and tc:GetSummonType()==SUMMON_TYPE_SYNCHRO
end
function c101010224.drop(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	if eg:IsExists(c101010224.filter,1,nil,eg) then
