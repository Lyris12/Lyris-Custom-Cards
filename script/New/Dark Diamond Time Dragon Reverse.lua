...
function c101010275.initial_effect(c)
	...
	e1:SetCondition(c101010275.fscon)
	e1:SetOperation(c101010275.fsop)
	c:RegisterEffect(e1)
	--LIGHT Machine
	...
	e3:SetTarget(c101010275.f3filter1)
	e3:SetValue(c101010275.efilter)
	c:RegisterEffect(e3)
	--Absolute Rewind
	...
	e4:SetCountLimit(1)
	e4:SetCost(c101010275.descost)
	e4:SetTarget(c101010275.destg)
	e4:SetOperation(c101010275.desop)
	c:RegisterEffect(e4)
end
function c101010275.f3filter1(e,c)
	return c:IsSetCard(0x5d) and c:IsType(TYPE_SPELL)
end
function c101010275.atcon(e)
	return e:GetHandler()
end
function c101010275.efilter(e,te)
	return te:GetOwnerPlayer()~=e:GetHandlerPlayer()
end
function c101010275.ffilter(c)
	return c:IsRace(RACE_MACHINE) or c:IsRace(RACE_DRAGON) or (c:IsSetCard(0x5d) and c:IsType(TYPE_SPELL))
end
function c101010275.f1filter(c)
	return c:IsRace(RACE_DRAGON)
end
function c101010275.f2filter(c)
	return c:IsRace(RACE_MACHINE)
end
function c101010275.f3filter(c)
	return c:IsSetCard(0x5d) and c:IsType(TYPE_SPELL)
end
function c101010275.fscon(e,g,gc,chkf)
	...
	if not gc then
		...
		local sg=Duel.GetMatchingGroup(c101010275.f3filter,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	...
	if gc then
		...
		local tc=g:GetFirst()
		while tc do
			if c101010275.f1filter(tc) then b1=1
				elseif c101010275.f2filter(tc) then b2=1
				elseif c101010275.f3filter(tc) then b3=1
			end
			tc=g:GetNext()
		end
		if c101010275.f1filter(gc) then b1=1
			elseif c101010275.f2filter(gc) then b2=1
			elseif c101010275.f3filter(gc) then b3=1
			else return false
		end
		return b1+b2+b3>2
	end
	...
	local tc=g:GetFirst()
	while tc do
		if c101010275.f1filter(tc) then b1=1 if aux.FConditionCheckF(tc,chkf) then fs=true end end
		if c101010275.f2filter(tc) then b2=1 if aux.FConditionCheckF(tc,chkf) then fs=true end end
		if c101010275.f3filter(tc) then b3=1 if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
	return b1+b2+b3>=3 and (fs or chkf==PLAYER_NONE)
end
function c101010275.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	local loc1=LOCATION_SZONE
	local loc2=LOCATION_SZONE
	...
	local mg=Duel.GetMatchingGroup(c101010275.f3filter,e:GetHandlerPlayer(),loc1,loc2,nil)
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	g:Merge(mg)
	if gc then
		local sg=eg:Filter(c101010275.ffilter,gc)
	...
	local sg=eg:Filter(c101010275.ffilter,nil)
	local g1=nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	else sg:Remove(c101010275.f3filter,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	local g2=sg:Select(tp,1,1,nil)
...
function c101010275.descost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010275.destg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010275.desop(e,tp,eg,ep,ev,re,r,rp)
