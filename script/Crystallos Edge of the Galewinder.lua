--烈風のクリスタロス・エッジ
function c101010534.initial_effect(c)
	--Activate
	...
	e1:SetCondition(c101010534.con)
	e1:SetTarget(c101010534.tg)
	e1:SetOperation(c101010534.op)
	c:RegisterEffect(e1)
	--Unlocked by a "Synchro" Synchro Monster
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE)
	e2:SetCode(EFFECT_QP_ACT_IN_NTPHAND)
	e2:SetCondition(c101010534.handcon)
	c:RegisterEffect(e2)
end
function c101010534.cfilter1(c)
	return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsRace(RACE_DRAGON)
end
function c101010534.con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c101010534.cfilter1,tp,LOCATION_MZONE,0,1,nil)
end
function c101010534.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010534.op(e,tp,eg,ep,ev,re,r,rp)
...
function c101010534.cfilter2(c)
	return c:IsFaceup() and (c:IsCode(82044279) or c:IsCode(50954680) or c:IsSetCard(0x17) or c:IsSetCard(0x92b))
end
function c101010534.handcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c101010534.cfilter2,tp,LOCATION_MZONE,0,1,nil)
end
