...
c:EnableReviveLimit()
	...
	fus:SetDescription(aux.Stringid(101010113,1))
	...
	fus:SetCondition(c101010113.sprcon)
	fus:SetOperation(c101010113.sprop)
	...
	e0:SetCountLimit(1,101010113)
	e0:SetCondition(c101010113.con1)
	e0:SetTarget(c101010113.target)
	e0:SetOperation(c101010113.op1)
	c:RegisterEffect(e0)
end
c101010113.material_count=2
c101010113.material={86686671,12670770}
...
function c101010113.fmcon(e,g,gc,chkf)
	...
	local sg=Duel.GetMatchingGroup(c101010113.ffilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	...
	if gc then
		...
		while tc do
			if tc:IsFusionCode(70095154) or tc:IsFusionCode(101010024) then b1=1
			elseif tc:GetCode()==86686671 then b2=1
			elseif tc:GetCode()==12670770 then b3=1
			end
			tc=mg:GetNext()
		end
		if gc:IsFusionCode(70095154) or gc:IsFusionCode(101010024) then b1=1
	...
	local tc=mg:GetFirst()
	while tc do
		if tc:IsFusionCode(70095154) or tc:IsFusionCode(101010024) then b1=1 if aux.FConditionCheckF(tc,chkf) then fs=true end
...
function c101010113.fsfilter(c)
	local code=c:GetCode()
	return (c:IsFusionCode(70095154) or c:IsFusionCode(101010024)) or code==86686671 or code==12670770
end
function c101010113.fmatl(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	...
	local mg=Duel.GetMatchingGroup(c101010113.ffilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	g:Merge(mg)
	if gc then
		local sg=g:Filter(c101010113.fsfilter,gc)
		if c101010113.code(gc) then sg:Remove(c101010113.code,nil) else  sg:Remove(Card.IsCode,nil,gc:GetCode()) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
		local g1=sg:Select(tp,1,1,nil)
		if c101010113.code(g1:GetFirst()) then sg:Remove(c101010113.code,nil) else sg:Remove(Card.IsCode,nil,g1:GetFirst():GetCode()) end
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	local sg=g:Filter(c101010113.fsfilter,nil)
	local g1=nil
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	local g2=sg:Select(tp,1,1,nil)
	if c101010113.code(g2:GetFirst()) then sg:Remove(c101010113.code,nil) else sg:Remove(Card.IsCode,nil,g2:GetFirst():GetCode()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
...
function c101010113.fdfilter(c)
	return c:IsLocation(LOCATION_SZONE) and c:IsFacedown()
end
function c101010113.sprfilter(c,tc)
	local code=c:GetCode()
	return ((code==70095154 or code==101010024) and c:IsCanBeXyzMaterial(tc)) or code==86686671 or code==12670770
end
function c101010113.sprcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c101010113.sprfilter,tp,LOCATION_HAND+LOCATION_ONFIELD,0,nil,e:GetHandler())
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and g:IsExists(c101010113.code,1,nil)
...
function c101010113.sprop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(c101010113.sprfilter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,nil)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
	if ft<=0 then g1=g:FilterSelect(tp,Card.IsLocation,1,1,nil,LOCATION_MZONE)
	else g1=g:Select(tp,1,1,nil) end
	if c101010113.code(g1:GetFirst()) then g:Remove(c101010113.code,nil) else g:Remove(Card.IsCode,nil,g1:GetFirst():GetCode()) end
	for i=1,2 do
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
		local g2=g:Select(tp,1,1,nil)
		if c101010113.code(g2:GetFirst()) then g:Remove(c101010113.code,nil) else g:Remove(Card.IsCode,nil,g2:GetFirst():GetCode()) end
		g1:Merge(g2)
	end
	local cg=g1:Filter(c101010113.filter,nil)
	if cg:GetCount()>0 then Duel.ConfirmCards(1-tp,cg) Duel.ShuffleHand(tp) end
	e:GetHandler():SetMaterial(g1)
end
function c101010113.cfilter(c)
	return c:IsFaceup() and c:IsLevelAbove(5)
end
function c101010113.xfilter(c)
	local code=c:GetCode()
	return ((code==70095154 or code==101010024) and c:IsCanBeXyzMaterial(c,true)) or code==86686671 or code==12670770
end
function c101010113.con1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return eg:IsExists(c101010113.cfilter,1,nil)
		and Duel.GetFieldGroupCount(tp,LOCATION_MZONE,0)==0
		and Duel.GetMatchingGroup(c101010113.xfilter,tp,LOCATION_HAND,0,nil):GetClassCount(Card.GetCode)>1
end
function c101010113.spfilter(c,e,tp)
	return c:IsType(TYPE_XYZ) and c:IsAttribute(ATTRIBUTE_LIGHT) and c:IsRace(RACE_MACHINE) and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_XYZ,tp,false,false) and c:GetCode()~=101010113
end
function c101010113.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return c:IsAbleToDeck() and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c101010113.spfilter,tp,LOCATION_EXTRA,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_EXTRA)
	Duel.SetOperationInfo(0,CATEGORY_TODECK,c,1,0,0)
end
function c101010113.op1(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c101010113.xfilter,tp,LOCATION_HAND,0,nil)
	local sg=Duel.GetMatchingGroup(c101010113.spfilter,tp,LOCATION_EXTRA,0,nil,e,tp)
	...
	while g:GetClassCount(Card.GetCode)>0 do
		...
		if g:GetCount()>0 and cc==0 and Duel.SelectYesNo(tp,aux.Stringid(101010113,7)) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
