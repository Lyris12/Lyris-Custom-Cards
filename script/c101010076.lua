...
aux.EnablePendulumAttribute(c)
	...
	e1:SetOperation(c101010076.spcon)
	c:RegisterEffect(e1)
	local e2=Effect.CreateEffect(c)
	e2:SetCategory(CATEGORY_SPECIAL_SUMMON)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_TRIGGER_O)
	e2:SetCode(EVENT_CUSTOM+101010076)
...
function c101010076.sumtg(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	if chk==0 then return e:GetHandler():IsRelateToEffect(e) and not e:GetHandler():IsStatus(STATUS_CHAINING)
		...
		and Duel.IsExistingMatchingCard(c101010076.filter,p,LOCATION_DECK,0,1,nil,e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c101010076.sumop(e,tp,eg,ep,ev,re,r,rp)
	local mg=e:GetLabelObject():GetLabelObject():Filter(c101010076.cfilter,nil,tp)
	local p=e:GetHandler():GetControler()
	local sg=Duel.GetMatchingGroup(c101010076.filter,p,LOCATION_DECK,0,nil,e,tp)
