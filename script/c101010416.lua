...
function c101010416.initial_effect(c)
	...
	e1:SetCondition(c101010416.fscon)
	e1:SetOperation(c101010416.fsop)
	c:RegisterEffect(e1)
	--equip
...
function c101010416.brfilter(c)
	return c:IsType(TYPE_SPELL) and c:IsSetCard(0x5d)
end
function c101010416.fscon(e,g,gc,chkf)
	...
	if not gc then
		...
		local sg=Duel.GetMatchingGroup(c101010416.brfilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local mg=g:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if sg then mg:Merge(sg) end
	if gc then
		if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
		if (gc:IsFusionCode(40418351) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(c101010416.brfilter,1,gc) then
			return true
			elseif c101010416.brfilter(gc) then
			...
			while tc do
				...
				if c101010416.brfilter(tc) then g2:AddCard(tc) end
				tc=mg:GetNext()
			end
			g1:RemoveCard(gc)
	...
	while tc do
		...
		local c2=c101010416.brfilter(tc)
		if c1 or c2 then
...
function c101010416.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if not gc then
		local loc1=LOCATION_SZONE
		local loc2=LOCATION_SZONE
		...
		local mg=Duel.GetMatchingGroup(c101010416.brfilter,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if mg then g:Merge(mg) end
	if gc then
		if (gc:IsFusionCode(40418351) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and g:IsExists(c101010416.brfilter,1,gc) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local g1=g:FilterSelect(tp,c101010416.brfilter,1,1,gc)
			...
			while tc do
				...
				if c101010416.brfilter(tc) then sg2:AddCard(tc) end
				tc=g:GetNext()
			end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	while tc do
		if tc:IsFusionCode(40418351) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then sg1:AddCard(tc) end
		if c101010416.brfilter(tc) then sg2:AddCard(tc) if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
