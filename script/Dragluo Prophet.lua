...
function c101010194.initial_effect(c)
	...
	e1:SetTarget(c101010194.splimit)
	c:RegisterEffect(e1)
	...
	e2:SetCountLimit(1,101010194)
	e2:SetTarget(c101010194.psptg)
	e2:SetOperation(c101010194.pspop)
	c:RegisterEffect(e2)
	...
	e4:SetCondition(c101010194.con)
	e4:SetCost(c101010194.pcost)
	e4:SetTarget(c101010194.tg)
	e4:SetOperation(c101010194.op)
	c:RegisterEffect(e4)
...
function c101010194.pcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	e1:SetTargetRange(1,0)
	e1:SetTarget(c101010194.sumlimit)
	Duel.RegisterEffect(e1,c:GetControler())
end
function c101010194.sumlimit(e,c,sump,sumtype,sumpos,targetp,se)
	return c:GetRace()~=RACE_DRAGON
end
function c101010194.pspfilter(c,e,tp)
	return c:IsType(TYPE_XYZ) and c:IsRace(RACE_DRAGON) and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c101010194.psptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_GRAVE) and chkc:IsControler(tp) and c101010194.pspfilter(chkc,e,tp) end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c101010194.pspfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local sg=Duel.SelectTarget(tp,c101010194.pspfilter,tp,LOCATION_GRAVE,0,1,1,nil,e,tp)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,sg,1,0,0)
end
function c101010194.pspop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c101010194.spfilter(c)
	return c:IsFaceup() and c:IsSetCard(0x94b) and c:GetLevel()==4
end
function c101010194.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(tp) and c101010194.spfilter(chkc) end
	if chk==0 then return true end
	if Duel.IsExistingTarget(c101010194.spfilter,tp,LOCATION_MZONE,0,1,nil) and Duel.GetFlagEffect(tp,101010194)==1 and Duel.SelectYesNo(tp,aux.Stringid(101010194,0)) then
		e:SetProperty(EFFECT_FLAG_CARD_TARGET)
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
		Duel.SelectTarget(tp,c101010194.spfilter,tp,LOCATION_MZONE,0,1,1,nil)
		Duel.RegisterFlagEffect(tp,101010194,RESET_PHASE+PHASE_END,0,1)
	else e:SetProperty(0) end
end
function c101010194.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=Duel.GetFirstTarget()
...
function c101010194.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010194.sptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingTarget(c101010194.sfilter,tp,LOCATION_GRAVE,0,1,nil,e,tp) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
