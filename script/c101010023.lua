...
--equip
	...
	e0:SetTarget(c101010023.eqtg)
	e0:SetOperation(c101010023.eqop)
	c:RegisterEffect(e0)
	--Do Not Remove
...
function c101010023.filter(c)
	return c:IsFaceup() and c:IsRace(RACE_MACHINE) and c:GetCode()~=101010023
end
function c101010023.eqtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() end
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_SZONE)>0
		and Duel.IsExistingTarget(c101010023.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_EQUIP)
	Duel.SelectTarget(tp,c101010023.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
end
function c101010023.eqop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	...
	if c:IsRelateToEffect(e) and tc:IsFaceup() and tc:IsRelateToEffect(e) then
		...
		e2:SetValue(c101010023.eqlimit)
		c:RegisterEffect(e2)
	end
end
function c101010023.eqlimit(e,c)
	return c:IsRace(RACE_MACHINE)
end
function c101010023.matcon(e)
	local c=e:GetHandler()
