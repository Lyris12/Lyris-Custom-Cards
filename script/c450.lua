...
function c450.op(e,tp,eg,ep,ev,re,r,rp)
	...
	while tc do
		if tc:GetFlagEffect(450)==0 then
			...
			tc:RegisterFlagEffect(450,RESET_EVENT+EVENT_ADJUST,0,1)  
		end
		tc=g:GetNext()
	end
end
...
function c450.subsfilter1(c,x,g,f1,f2)
	...
	local lv=c:GetLevel()
	local rk=c:GetRank()
	return g:IsExists(c450.subsfilter2,1,c,x,lv,rk,f2)
end
function c450.subsfilter2(c,x,lv,rk,f)
	return math.abs(c:GetLevel()-lv==x) or math.abs(c:GetLevel()-rk)==x and (not f or f(c))
end
function c450.sspcon(e,c)
	...
	return g:IsExists(c450.subsfilter1,1,nil,c:GetOriginalRank(),g) and c:GetFlagEffect(743000003)==0
end
function c450.sspop(e,tp,eg,ep,ev,re,r,rp,c)
	...
	local mg=Group.CreateGroup()
	local x=c:GetOriginalRank()
	local g=Duel.GetMatchingGroup(c450.subfilter,tp,LOCATION_MZONE,0,nil,x,c.material1,c.material2)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
	local tc1=g:FilterSelect(tp,c450.subsfilter1,1,1,nil,x,g):GetFirst()
	mg:AddCard(tc1)
	if c.material1(tc1) and not c.material2(tc1) then g:Remove(c450.srfilter,nil,c.material1,c.material2) end
	if c.material2(tc1) and not c.material1(tc1) then g:Remove(c450.srfilter,nil,c.material2,c.material1) end
	local tc2=g:FilterSelect(tp,c450.subsfilter2,1,1,tc1,x,tc1:GetLevel(),tc1:GetRank())
	mg:AddCard(tc2)
	c:SetMaterial(mg)
	local fg=mg:Filter(Card.IsFacedown,nil)
	if fg:GetCount()>0 then Duel.ConfirmCards(1-tp,fg) end
...
function c450.mltfilter(c,f)
	return c:IsAbleToRemoveAsCost() and ((not f1 or f1(c)) or (not f2 or f2(c)))
end
function c450.mspcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-2 and Duel.IsExistingMatchingCard(c450.mltfilter,tp,LOCATION_MZONE,0,2,nil,c.material1,c.material2) and c:GetFlagEffect(743000003)==0
end
function c450.mspop(e,tp,eg,ep,ev,re,r,rp,c)
	local ec=Duel.GetMatchingGroup(c450.regfilter,tp,LOCATION_MZONE,0,nil)
	if ec:GetCount()>0 then Duel.Remove(ec,POS_FACEUP,REASON_EFFECT) end
	local g=Duel.GetMatchingGroup(c450.mltfilter,tp,LOCATION_MZONE,0,nil,c.material1,c.material2)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_REMOVE)
