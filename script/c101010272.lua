...
c:EnableReviveLimit()
	...
	e5:SetCondition(c101010272.cbcon)
	e5:SetTarget(c101010272.cbtg)
	e5:SetOperation(c101010272.cbop)
	c:RegisterEffect(e5)
	...
	e6:SetCondition(c101010272.dcon)
	e6:SetValue(2000)
	c:RegisterEffect(e6)
end
...
function c101010272.cfilter(c)
	return c:IsReleasableByEffect() and (c:IsLevelBelow(11) or (c:IsCode(101010187) and c:IsHasEffect(101010187)))
end
...
function c101010272.actcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local g=nil
	if not cg:CheckWithSumEqual(Card.GetLevel,12,1,99) or (cg:IsExists(c101010272.star,1,nil) and Duel.SelectYesNo(tp,aux.Stringid(101010272,0))) then
...
function c101010272.after(e,tp)
...
function c101010272.cbcon(e,tp,eg,ep,ev,re,r,rp)
	return r~=REASON_REPLACE and eg:GetFirst():GetControler()==e:GetHandler():GetControler()
end
function c101010272.cbfilter(c)
	return c:IsFacedown() or c:IsSetCard(0x9008)
end
function c101010272.cbtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_ONFIELD) and chkc:IsControler(tp) and c101010272.cbfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101010272.cbfilter,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c101010272.cbfilter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c101010272.cbop(e,tp,eg,ep,ev,re,r,rp)
...
function c101010272.cefilter(c,ev)
	local g=Duel.GetChainInfo(ev,CHAININFO_TARGET_CARDS)
...
function c101010272.cetg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if chkc then return chkc:IsLocation(g:GetFirst():GetLocation()) and chkc:IsControler(tp) and c101010272.cefilter(chkc,ev) end
	if chk==0 then return Duel.IsExistingTarget(c101010272.cefilter,tp,g:GetFirst():GetLocation(),0,1,g:GetFirst(),ev) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,c101010272.cefilter,tp,g:GetFirst():GetLocation(),0,1,1,g:GetFirst(),ev)
end
...
function c101010272.dcon(e,tp,eg,ep,ev,re,r,rp)
