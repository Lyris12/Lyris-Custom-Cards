...
function c101010301.initial_effect(c)
	...
	e5:SetValue(c101010301.splimit)
	c:RegisterEffect(e5)
	--fusion material
	...
	e1:SetCondition(c101010301.fscondition)
	e1:SetOperation(c101010301.fsoperation)
	c:RegisterEffect(e1)
	--special summon
	...
	e2:SetCondition(c101010301.spcon)
	e2:SetOperation(c101010301.spop)
	...
	e4:SetOperation(c101010301.effop)
	c:RegisterEffect(e4)
	local e3=Effect.CreateEffect(c)
	e3:SetType(EFFECT_TYPE_SINGLE)
	e3:SetCode(EFFECT_MATERIAL_CHECK)
	e3:SetValue(c101010301.matcheck)
	e3:SetLabelObject(e4)
	c:RegisterEffect(e3)
end
function c101010301.splimit(e,se,sp,st)
	return bit.band(st,SUMMON_TYPE_FUSION)==SUMMON_TYPE_FUSION
end
function c101010301.fsfilter(c,mg)
	return c:IsSetCard(0x34)
end
function c101010301.fscondition(e,mg,gc)
	if mg==nil then return false end
	if gc then return false end
	return mg:IsExists(c101010301.fsfilter,2,nil,mg)
end
function c101010301.fsoperation(e,tp,eg,ep,ev,re,r,rp,gc)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	local g=eg:FilterSelect(tp,c101010301.fsfilter,2,63,nil,eg)
	Duel.SetFusionMaterial(g)
end
function c101010301.spfilter(c)
	return c:IsSetCard(0x34) and (not c:IsOnField() or c:IsFaceup()) and c:IsAbleToRemoveAsCost()
end
function c101010301.spcon(e,c)
	if c==nil then return true end
	if Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)<=0 then return false end
	local g=Duel.GetMatchingGroup(c101010301.spfilter,c:GetControler(),LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
	local ct=g:GetClassCount(Card.GetCode)
	return ct>6
end
function c101010301.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(c101010301.spfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
...
function c101010301.matcheck(e,c)
	local ct=e:GetHandler():GetMaterial():GetClassCount(Card.GetCode)
	e:GetLabelObject():SetLabel(ct)
end
function c101010301.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct~=0 then
		...
		if ct>=1 then
			...
			e1:SetCondition(c101010301.atcon)
			e1:SetCost(c101010301.atcost)
			e1:SetOperation(c101010301.atop)
			c:RegisterEffect(e1)
		end
		if ct>=2 then
			local e2=Effect.CreateEffect(c)
			...
			e9:SetOperation(c101010301.oaop)
			c:RegisterEffect(e9)
		end
		if ct>=4 then
			local e3=Effect.CreateEffect(c)
			...
			e3:SetCondition(c101010301.bdrcon)
			e3:SetOperation(c101010301.bdrop)
			c:RegisterEffect(e3)
		end
		if ct>=5 then
			local e4=Effect.CreateEffect(c)
			...
			e4:SetValue(c101010301.tgvalue)
			c:RegisterEffect(e4)
		end
		if ct>=7 then
			local e5=Effect.CreateEffect(c)
			...
			e5:SetCost(c101010301.tdcost)
			e5:SetTarget(c101010301.tdtg)
			e5:SetOperation(c101010301.tdop)
			c:RegisterEffect(e5)
		end
		else
		...
		e6:SetTarget(c101010301.desreptg)
		e6:SetOperation(c101010301.desrepop)
		c:RegisterEffect(e6)
	end
end
function c101010301.atcon(e,tp,eg,ep,ev,re,r,rp)
	local phase=Duel.GetCurrentPhase()
	return phase~=PHASE_DAMAGE or not Duel.IsDamageCalculated()
end
function c101010301.afilter(c)
	return c:IsFaceup() and c:IsSetCard(0x34) and c:IsAbleToGraveAsCost()
end
function c101010301.atcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010301.afilter,tp,LOCATION_ONFIELD,0,1,nil) end
	local g=Duel.GetMatchingGroup(c101010301.afilter,tp,LOCATION_ONFIELD,0,nil)
	Duel.Remove(g,POS_FACEUP,REASON_COST)
	e:SetLabel(g:GetCount())
end
function c101010301.atop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c101010301.cfilter(c)
	return c:IsSetCard(0x34) and (c:IsType(TYPE_XYZ) and c:IsAbleToExtraAsCost()) or c:IsAbleToDeckAsCost()
end
function c101010301.tdcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010301.cfilter,tp,LOCATION_REMOVED,0,5,nil) end
	local g=Duel.GetMatchingGroup(c101010301.cfilter,tp,LOCATION_REMOVED,0,nil)
	Duel.SendtoDeck(g,nil,2,REASON_COST)
end
function c101010301.tdtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010301.tdop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.GetMatchingGroup(Card.IsAbleToDeck,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,e:GetHandler())
	Duel.SendtoDeck(g,nil,2,REASON_EFFECT)
end
function c101010301.bdrcon(e,tp,eg,ep,ev,re,r,rp)
	local at=Duel.GetAttackTarget()
	return at~=nil and at:GetControler()==tp
end
function c101010301.dircon(e)
	return e:GetHandler():GetAttackAnnouncedCount()>0
end
function c101010301.atkcon(e)
	return e:GetHandler():IsDirectAttacked()
end
function c101010301.cfilter2(c)
	return c:IsSetCard(0x34) and c:IsAbleToRemoveAsCost()
end
function c101010301.desreptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010301.cfilter2,tp,LOCATION_GRAVE,0,1,nil) end
	return Duel.SelectYesNo(tp,aux.Stringid(101010301,1))
end
function c101010301.desrepop(e,tp,eg,ep,ev,re,r,rp)
	local g=Duel.SelectMatchingCard(tp,c101010301.cfilter2,tp,LOCATION_GRAVE,0,1,1,nil)
	Duel.Remove(g,POS_FACEUP,REASON_EFFECT+REASON_REPLACE)
end
function c101010301.tgvalue(e,re,rp)
	return rp~=e:GetHandlerPlayer()
end
function c101010301.oaop(e,tp,eg,ep,ev,re,r,rp)
...
function c101010301.bdrop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
