...
function ref.spcon(e,c)
	...
	local g=Duel.GetMatchingGroup(ref.spfilter,c:GetControler(),LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
	local ct=g:GetClassCount(Card.GetCode)
	return ct>6
end
function ref.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.GetMatchingGroup(ref.spfilter,tp,LOCATION_ONFIELD+LOCATION_GRAVE,0,nil)
...
function ref.effop(e,tp,eg,ep,ev,re,r,rp)
	...
	if ct~=0 then
		...
		if ct>=4 then
			...
			e3:SetCondition(ref.bdrcon)
			e3:SetOperation(ref.bdrop)
			c:RegisterEffect(e3)
		end
		if ct>=5 then
			local e4=Effect.CreateEffect(c)
		...
		e6:SetTarget(ref.desreptg)
		e6:SetOperation(ref.desrepop)
		c:RegisterEffect(e6)
	end
end
...
function ref.atcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(ref.afilter,tp,LOCATION_ONFIELD,0,1,nil) end
	local g=Duel.GetMatchingGroup(ref.afilter,tp,LOCATION_ONFIELD,0,nil)
...
function ref.tdcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(ref.cfilter,tp,LOCATION_REMOVED,0,5,nil) end
	local g=Duel.GetMatchingGroup(ref.cfilter,tp,LOCATION_REMOVED,0,nil)
...
function ref.bdrcon(e,tp,eg,ep,ev,re,r,rp)
...
function ref.bdrop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
