...
function c101010673.initial_effect(c)
	...
	e4:SetTarget(c101010673.reptg)
	e4:SetOperation(c101010673.repop)
	e4:SetValue(c101010673.repval)
	c:RegisterEffect(e4)
	--
	local e2=Effect.CreateEffect(c)
	e2:SetType(EFFECT_TYPE_SINGLE+EFFECT_TYPE_CONTINUOUS)
	e2:SetCode(EVENT_REMOVE)
	e2:SetOperation(c101010673.tgop)
	c:RegisterEffect(e2)
end
function c101010673.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsOnField()
		and c:IsSetCard(0x127) and c:IsReason(REASON_BATTLE+REASON_EFFECT)
end
function c101010673.cfilter(c)
	return c:IsSetCard(0x127) and c:IsAbleToGrave()
end
function c101010673.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local pg=eg:Filter(c101010673.repfilter,nil,tp)
	if chk==0 then return pg:GetCount()>0 and Duel.IsExistingMatchingCard(c101010673.cfilter,tp,LOCATION_DECK,0,1,nil) and not pg:IsContains(e:GetHandler()) end
	if Duel.SelectYesNo(tp,aux.Stringid(101010673,0)) then
		local ct=pg:GetCount()
		local rg=Duel.SelectMatchingCard(tp,c101010673.cfilter,tp,LOCATION_DECK,0,1,ct,nil)
...
function c101010673.repval(e,c)
	return e:GetLabelObject():IsContains(c)
end
function c101010673.tgop(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsReason(REASON_EFFECT) and c:GetReasonEffect():IsSetCard(0x127) then
		...
		e2:SetOperation(c101010673.regop)
		...
		e3:SetCondition(c101010673.damcon)
		e3:SetOperation(c101010673.damop)
		e3:SetReset(RESET_EVENT+0x1fe0000)
		c:RegisterEffect(e3)
	end
end
function c101010673.regop(e,tp,eg,ep,ev,re,r,rp)
	e:GetHandler():RegisterFlagEffect(101010673,RESET_EVENT+0x1fc0000+RESET_CHAIN,0,1)
end
function c101010673.damcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c101010673.damop(e,tp,eg,ep,ev,re,r,rp)
