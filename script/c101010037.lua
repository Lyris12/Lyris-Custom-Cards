...
--fusion material
	c:EnableReviveLimit()
	aux.AddFusionProcCodeFun(c,id,aux.FilterBoolFunction(Card.IsSetCard,0x4093),2,true,true)
	--add LIGHT attribute
	...
	e4:SetCondition(ref.dmgcon)
	e4:SetTarget(ref.dmgtg)
	e4:SetOperation(ref.dmgop)
	c:RegisterEffect(e4)
end
function ref.splimit(e,se,sp,st)
	return st==SUMMON_TYPE_FUSION+0xFA0
end
function ref.dmgcon(e,tp,eg,ep,ev,re,r,rp)
	return ep~=tp
end
function ref.dmgtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
