...
function c101010072.initial_effect(c)
	...
	e1:SetCondition(c101010072.con)
	e1:SetTarget(c101010072.tg)
	e1:SetOperation(c101010072.act)
	c:RegisterEffect(e1)
end
function c101010072.cfilter1(c)
	return c:IsFaceup() and c:IsType(TYPE_SYNCHRO) and c:IsRace(RACE_DRAGON)
end
function c101010072.cfilter2(c)
	return c:IsFaceup() and (c:IsCode(82044279) or c:IsCode(50954680) or c:IsSetCard(0x17) or c:IsSetCard(0x92b))
end
function c101010072.con(e,tp,eg,ep,ev,re,r,rp)
	return Duel.IsExistingMatchingCard(c101010072.cfilter1,tp,LOCATION_MZONE,0,1,nil)
	and (Duel.GetCurrentPhase()==PHASE_MAIN1 or Duel.GetCurrentPhase()==PHASE_MAIN2 or Duel.IsExistingMatchingCard(c101010072.cfilter2,tp,LOCATION_ONFIELD,0,1,nil))
end
function c101010072.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010072.act(e,tp,eg,ep,ev,re,r,rp)
