...
function c101010461.initial_effect(c)
	...
	e1:SetTarget(c101010461.thtg)
	e1:SetOperation(c101010461.thop)
	c:RegisterEffect(e1)
	--atk down
	...
	e2:SetCondition(c101010461.adcon)
	e2:SetTarget(c101010461.adtg)
	e2:SetOperation(c101010461.adop)
	c:RegisterEffect(e2)
end
function c101010461.filter(c)
	return c:IsType(TYPE_MONSTER) and c:IsSetCard(0x2ea) and c:GetCode()~=101010461 and c:IsAbleToHand()
end
...
function c101010461.adcon(e,tp,eg,ep,ev,re,r,rp)
	local rc=e:GetHandler():GetReasonCard()
	return r==REASON_RITUAL and rc:IsSetCard(0x2ea)
end
function c101010461.adtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsControler(1-tp) end
	if chk==0 then return Duel.IsExistingTarget(nil,tp,0,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	Duel.SelectTarget(tp,nil,tp,0,LOCATION_MZONE,1,1,nil)
end
function c101010461.adop(e,tp,eg,ep,ev,re,r,rp)
