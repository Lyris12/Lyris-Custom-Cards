...
function c101010161.initial_effect(c)
	...
	e1:SetCondition(c101010161.fscon)
	e1:SetOperation(c101010161.fsop)
	c:RegisterEffect(e1)
	--destroy
	...
	e4:SetRange(0x8f)
	e4:SetOperation(c101010161.desop2)
	c:RegisterEffect(e4)
	local e5=e4:Clone()
	...
	e2:SetCondition(c101010161.actcon)
	e2:SetTarget(c101010161.acttg)
	e2:SetOperation(c101010161.act)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e7:SetCondition(c101010161.con)
	e7:SetTarget(c101010161.tg)
	e7:SetOperation(c101010161.op)
	c:RegisterEffect(e7)
end
function c101010161.material(c)
	return c:IsType(TYPE_SPELL) and c:IsSetCard(0x5d)
end
function c101010161.fscon(e,g,gc,chkf)
	...
	if not gc then
		...
		local sg=Duel.GetMatchingGroup(c101010161.material,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local mg=g:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if sg then mg:Merge(sg) end
	if gc then
		if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return false end
		if (gc:IsCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and mg:IsExists(c101010161.material,1,gc) then
			return true
		elseif c101010161.material(gc) then
	...
	while tc do
		...
		local c2=c101010161.material(tc)
		if c1 or c2 then
...
function c101010161.fsop(e,tp,eg,ep,ev,re,r,rp,gc,chkf)
	if not gc then
		local loc1=LOCATION_SZONE
		local loc2=LOCATION_SZONE
		...
		local mg=Duel.GetMatchingGroup(c101010161.material,e:GetHandlerPlayer(),loc1,loc2,nil)
	end
	local g=eg:Filter(Card.IsCanBeFusionMaterial,nil,e:GetHandler())
	if mg then g:Merge(mg) end
	if gc then
		if not gc:IsCanBeFusionMaterial(e:GetHandler()) then return end
		if (gc:IsCode(76263644) or gc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE)) and g:IsExists(c101010161.material,1,gc) then
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
			local g1=g:FilterSelect(tp,c101010161.material,1,1,gc)
			...
			while tc do
				...
				if c101010161.material(tc) then sg2:AddCard(tc) end
				tc=g:GetNext()
			end
			Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FMATERIAL)
	...
	while tc do
		if tc:IsCode(76263644) or tc:IsHasEffect(EFFECT_FUSION_SUBSTITUTE) then sg1:AddCard(tc) end
		if c101010161.material(tc) then sg2:AddCard(tc) if aux.FConditionCheckF(tc,chkf) then fs=true end end
		tc=g:GetNext()
	end
...
function c101010161.desop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if not c:IsOnField() then
		Duel.Destroy(c,REASON_EFFECT)
	end
end
function c101010161.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c101010161.con(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():GetPreviousControler()==tp
end
function c101010161.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010161.op(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		Duel.SpecialSummon(c,0,tp,tp,false,false,POS_FACEUP_DEFENCE)
	end
end
function c101010161.actcon(e,tp,eg,ep,ev,re,r,rp)
...
function c101010161.acttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010161.act(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	if c:IsRelateToEffect(e) then
		...
		c101010161.after(e,tp)
	end
end
function c101010161.after(e,tp)
