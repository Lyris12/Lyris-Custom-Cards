...
function c101010418.initial_effect(c)
	...
	e0:SetTarget(c101010418.target)
	e0:SetOperation(c101010418.operation)
	c:RegisterEffect(e0)
	...
	e2:SetCondition(c101010418.damcon)
	e2:SetTarget(c101010418.damtg)
	e2:SetOperation(c101010418.damop)
	c:RegisterEffect(e2)
	--spsummon
	...
	e3:SetTarget(c101010418.sptg)
	e3:SetOperation(c101010418.spop)
	c:RegisterEffect(e3)
end
function c101010418.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101010418.operation(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFirstTarget()
	if e:GetHandler():IsRelateToEffect(e) and tc:IsRelateToEffect(e) and tc:IsFaceup() then
		Duel.Equip(tp,e:GetHandler(),tc)
	end
end
function c101010418.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():IsDiscardable() end
	Duel.SendtoGrave(e:GetHandler(),REASON_COST+REASON_DISCARD)
end
function c101010418.filter(c,e,tp,m)
...
function c101010418.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local mg=Duel.GetRitualMaterial(tp)
		return Duel.IsExistingMatchingCard(c101010418.filter,tp,LOCATION_HAND,0,1,nil,e,tp,mg)
	end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_HAND)
end
function c101010418.spop(e,tp,eg,ep,ev,re,r,rp)
	local mg=Duel.GetRitualMaterial(tp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
	local tg=Duel.SelectMatchingCard(tp,c101010418.filter,tp,LOCATION_HAND,0,1,1,nil,e,tp,mg)
...
function c101010418.damcon(e,tp,eg,ep,ev,re,r,rp)
	local ec=e:GetHandler():GetEquipTarget()
	return ec and ec:GetControler()==tp and ec==Duel.GetAttacker() and ec:GetBattleTarget()
end
function c101010418.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010418.damop(e,tp,eg,ep,ev,re,r,rp)
	local tc=e:GetHandler():GetEquipTarget()
	local bc=tc:GetBattleTarget()
