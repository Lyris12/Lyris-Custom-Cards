...
function c101010407.initial_effect(c)
	--Xyz Summon
	c:EnableReviveLimit()
	...
	e1:SetTarget(c101010407.tg)
	e1:SetOperation(c101010407.op)
	c:RegisterEffect(e1)
	--detach
	...
	e2:SetCost(c101010407.cost)
	e2:SetTarget(c101010407.mptg)
	e2:SetOperation(c101010407.mpop)
	c:RegisterEffect(e2)
	--Do Not Remove
	...
	point:SetOperation(c101010407.ipoint)
	...
	if not relay_check then
		...
		ge1:SetOperation(c101010407.recount)
		Duel.RegisterEffect(ge1,0)
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ATTACK_DISABLED)
		ge2:SetOperation(c101010407.recheck)
		Duel.RegisterEffect(ge2,0)
		local ge3=Effect.CreateEffect(c)
		ge3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge3:SetCode(EVENT_BATTLED)
		ge3:SetOperation(c101010407.recount2)
	...
	ge4:SetCondition(c101010407.rescon)
	ge4:SetOperation(c101010407.resop)
	ge4:SetValue(0x8773)
	...
	ge5:SetTarget(c101010407.pointchk)
	ge5:SetOperation(c101010407.point)
...
function c101010407.ipoint(e,tp,eg,ep,ev,re,r,rp)
...
function c101010407.recount(e,tp,eg,ep,ev,re,r,rp)
...
function c101010407.recheck(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local ct=tc:GetFlagEffectLabel(10001000)
	if ct then
...
function c101010407.recount2(e,tp,eg,ep,ev,re,r,rp)
	Duel.RegisterFlagEffect(tp,10001000,RESET_PHASE+PHASE_END,0,1)
end
function c101010407.resfilter(c)
...
function c101010407.rescon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c101010407.resfilter,tp,0xbe,0,nil)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft<=-g:FilterCount(Card.IsLocation,nil,LOCATION_MZONE) then return false end
	return g:GetCount()>0 and Duel.IsExistingMatchingCard(c101010407.refilter,tp,LOCATION_EXTRA,0,1,nil,e,tp)
end
function c101010407.fdfilter(c)
	return c:IsFacedown() or c:IsLocation(LOCATION_HAND)
end
function c101010407.refilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_PENDULUM)
		and not c:IsCanBeSpecialSummoned(e,0,tp,false,false) and not c:IsForbidden()
end
function c101010407.refilter1(c,e,tp)
	return not c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ) and c101010407.refilter(c,e,tp)
end
function c101010407.refilter2(c,e,tp)
	return c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ) and c101010407.refilter(c,e,tp)
end
function c101010407.resop(e,tp,eg,ep,ev,re,r,rp,c,sg,og)
	local g=Duel.GetMatchingGroup(c101010407.resfilter,tp,0xbe,0,nil)
	local fg=g:Filter(c101010407.fdfilter,nil)
	...
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if Duel.IsExistingMatchingCard(c101010407.refilter1,tp,LOCATION_EXTRA,0,1,nil,e,tp) and Duel.SelectYesNo(tp,aux.Stringid(101010401,5)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local rg=Duel.SelectMatchingCard(tp,c101010407.refilter1,tp,LOCATION_EXTRA,0,1,ft,nil,e,tp)
		sg:Merge(rg)
	end
	if (not rg or ft>rg:GetCount()) and Duel.IsExistingMatchingCard(c101010407.refilter2,tp,LOCATION_EXTRA,0,1,nil,e,tp) and (not rg or Duel.SelectYesNo(tp,aux.Stringid(101010401,2))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local pc=Duel.SelectMatchingCard(tp,c101010407.refilter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
		sg:Merge(pc)
	end
	...
	while sc do
		...
		e1:SetCode(EVENT_SPSUMMON_SUCCESS)
		e1:SetLabel(pt1+pt2)
		e1:SetOperation(c101010407.pointop)
		sc:RegisterEffect(e1)
		sc=sg:GetNext()
	end
end
function c101010407.pointop(e,tp,eg,ep,ev,re,r,rp)
	local pt=e:GetLabel()
	local c=e:GetHandler()
	local point=c:GetFlagEffectLabel(10001100)
	if not point then
		c:RegisterFlagEffect(10001100,0,EFFECT_FLAG_CLIENT_HINT+EFFECT_FLAG_CANNOT_DISABLE+EFFECT_FLAG_UNCOPYABLE,1,pt,aux.Stringid(101010401,1))
	else
		c:SetFlagEffectLabel(10001100,point+pt)
	end
end
function c101010407.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c101010407.op(e,tp,eg,ep,ev,re,r,rp)
	Duel.DiscardDeck(1-tp,e:GetLabel(),REASON_EFFECT)
end
function c101010407.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c101010407.mptg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) end
	if chk==0 then return Duel.IsExistingTarget(aux.TRUE,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,aux.TRUE,tp,LOCATION_MZONE,0,1,1,nil)
end
function c101010407.mpop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc:IsRelateToEffect(e) then
		...
		e4:SetOperation(c101010407.disop)
		e4:SetReset(RESET_PHASE+PHASE_END+RESET_EVENT+0x1fe0000)
		tc:RegisterEffect(e4)
	end
end
function c101010407.disop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
