...
function c101010102.initial_effect(c)
	...
	e0:SetCondition(c101010102.descon)
	e0:SetOperation(c101010102.desop)
	c:RegisterEffect(e0)
	--destroy
	...
	e1:SetCondition(c101010102.con)
	e1:SetCost(c101010102.cost)
	e1:SetTarget(c101010102.tg)
	e1:SetOperation(c101010102.op)
	c:RegisterEffect(e1)
	--no battle damage
	...
	e2:SetOperation(c101010102.dop)
	c:RegisterEffect(e2)
end
...
function c101010102.dop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c101010102.con(e,tp,eg,ep,ev,re,r,rp)
	return bit.band(e:GetHandler():GetSummonType(),SUMMON_TYPE_XYZ)==SUMMON_TYPE_XYZ
end
function c101010102.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c101010102.filter(c)
	return c:IsFaceup() and c:IsType(TYPE_MONSTER) and c:IsAbleToGrave()
end
function c101010102.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	local ct=e:GetHandler():GetOverlayCount()-1
	if chkc then return false end
	if chk==0 then return true end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local g1=Duel.SelectTarget(tp,c101010102.filter,tp,LOCATION_REMOVED,LOCATION_REMOVED,1,1,nil)
...
function c101010102.op(e,tp,eg,ep,ev,re,r,rp)
