...
function c101010130.dfilter(c)
	return c:IsFacedown() and c:IsSetCard(0x9008)
end
function c101010130.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010130.dfilter,tp,LOCATION_SZONE,0,1,nil) end
...
function c101010130.rfilter(c,e,tp,m)
...
function c101010130.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local rg=Duel.SelectMatchingCard(tp,c101010130.dfilter,tp,LOCATION_SZONE,0,1,1,nil)
	if rg:GetCount()>0 then
		...
		local tg=Duel.SelectMatchingCard(tp,c101010130.rfilter,tp,LOCATION_HAND+LOCATION_SZONE,0,1,1,nil,e,tp,mg)
