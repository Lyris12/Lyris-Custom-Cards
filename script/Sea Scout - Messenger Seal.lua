...
function c101010208.initial_effect(c)
	...
	e1:SetCondition(c101010208.con1)
	e1:SetTarget(c101010208.tg)
	e1:SetOperation(c101010208.op)
	c:RegisterEffect(e1)
	local e0=e1:Clone()
	e0:SetCode(EVENT_TO_GRAVE)
	e0:SetCountLimit(1,EFFECT_COUNT_CODE_OATH)
	e0:SetCondition(c101010208.con2)
	c:RegisterEffect(e0)
	--draw/special summon
	...
	e2:SetCost(c101010208.spcost)
	e2:SetTarget(c101010208.sptg)
	e2:SetOperation(c101010208.spop)
	c:RegisterEffect(e2)
end
function c101010208.con1(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsLocation(LOCATION_GRAVE) and r==REASON_SYNCHRO
end
function c101010208.con2(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsReason(REASON_SYNCHRO)
end
function c101010208.filter(c,e,tp)
	return c:IsSetCard(0x5cd) and c:IsLevelBelow(4) and c:GetCode()~=101010208 and c:IsCanBeSpecialSummoned(e,0,tp,false,false)
end
function c101010208.tg(e,tp,eg,ep,ev,re,r,rp,chk)
	local rc=e:GetHandler():GetReasonCard()
	if chk==0 then return rc:IsCanBeEffectTarget(e) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c101010208.filter,tp,LOCATION_HAND,0,1,nil,e,tp) end
...
function c101010208.op(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.GetMatchingGroup(c101010208.filter,tp,LOCATION_HAND,0,nil,e,tp)
	if g:GetCount()>0 then
		...
		e1:SetCondition(c101010208.drcon)
		e1:SetTarget(c101010208.drtg)
		e1:SetOperation(c101010208.drop)
		rc:RegisterEffect(e1)
	end
end
function c101010208.drcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_CHAINING)
end
function c101010208.drtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
...
function c101010208.drop(e,tp,eg,ep,ev,re,r,rp)
	local p,d=Duel.GetChainInfo(0,CHAININFO_TARGET_PLAYER,CHAININFO_TARGET_PARAM)
	Duel.Draw(p,d,REASON_EFFECT)
end
function c101010208.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(Card.IsDiscardable,tp,LOCATION_HAND,0,1,nil) end
	Duel.DiscardHand(tp,Card.IsDiscardable,1,1,REASON_COST,nil)
end
function c101010208.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsPlayerCanDraw(tp,1)
		and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 end
end
function c101010208.spop(e,tp,eg,ep,ev,re,r,rp)
