...
function c101010057.initial_effect(c)
	...
	e2:SetTarget(c101010057.rectg)
	e2:SetOperation(c101010057.recop)
	c:RegisterEffect(e2)
end
...
function c101010057.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=Duel.GetMatchingGroupCount(c101010057.filter,tp,LOCATION_MZONE,0,nil)
	if chk==0 then return Duel.GetTurnPlayer()==tp and ct>0 end
	Duel.SetTargetPlayer(tp)
	local rec=ct*500
	Duel.SetOperationInfo(0,CATEGORY_RECOVER,nil,0,tp,rec)
end
function c101010057.recop(e,tp,eg,ep,ev,re,r,rp)
	local rec=Duel.GetMatchingGroupCount(c101010057.filter,tp,LOCATION_MZONE,0,nil)*500
