...
function c101010176.initial_effect(c)
	...
	e2:SetCondition(c101010176.actcon)
	e2:SetTarget(c101010176.acttg)
	e2:SetOperation(c101010176.act)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
...
function c101010176.actcon(e,tp,eg,ep,ev,re,r,rp)
	return not e:GetHandler():IsStatus(STATUS_SET_TURN) or e:GetHandler():GetFlagEffect(101010170)~=0
end
function c101010176.acttg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and e:GetHandler():IsCanBeSpecialSummoned(e,0,tp,false,false) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010176.act(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		...
		c101010176.after(e,tp)
	end
end
function c101010176.filter(c)
	return c:GetType()==0x82 and (c:IsLocation(LOCATION_REMOVED) or c:IsSetCard(0x8))
end
function c101010176.after(e,tp)
	local g=Duel.GetMatchingGroup(c101010176.filter,tp,LOCATION_DECK+LOCATION_REMOVED,0,nil)
