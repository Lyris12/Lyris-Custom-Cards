...
function c101010164.filter(c,tp,m1,m2)
...
function c101010164.mfilter1(c,tp)
	return c:GetLevel()>0 and c:IsAbleToRemove() and Duel.IsExistingMatchingCard(c101010164.mfilter2,tp,LOCATION_EXTRA,0,1,c,tp,c)
end
function c101010164.mfilter2(c,tp,fc)
	return c:GetRank()>0 and c:IsAbleToRemove() and g:IsExists(c101010164.filter,1,nil,tp,c,fc)
end
function c101010164.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>0
		and Duel.IsExistingMatchingCard(c101010164.mfilter1,tp,LOCATION_EXTRA,0,1,nil,tp) end
...
function c101010164.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local mg=Duel.SelectMatchingCard(tp,c101010164.mfilter1,tp,LOCATION_EXTRA,0,1,1,nil,tp)
	local mc1=mg:GetFirst()
	local mg2=Duel.SelectMatchingCard(tp,c101010164.mfilter2,tp,LOCATION_EXTRA,0,1,1,mc1,tp,mc1)
	...
	if sg:CheckWithSumEqual(Card.GetLevel,mc1:GetLevel()+mc2:GetRank(),1,ft) then
		...
		Duel.Remove(mat,REASON_EFFECT+REASON_MATERIAL+REASON_RITUAL)
		Duel.BreakEffect()
		tc=tg:GetFirst()
		while tc do
