...
function c101010402.initial_effect(c)
	...
	ae2:SetLabel(1)
	ae2:SetCost(c101010402.cost)
	ae2:SetTarget(c101010402.damtg)
	ae2:SetOperation(c101010402.damop)
	...
	point:SetOperation(c101010402.ipoint)
	...
	ge0:SetTarget(c101010402.reen)
	ge0:SetOperation(c101010402.reenop)
	...
	if not relay_check then
		...
		ge1:SetOperation(c101010402.recount)
		Duel.RegisterEffect(ge1,0)
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ATTACK_DISABLED)
		ge2:SetOperation(c101010402.recheck)
		Duel.RegisterEffect(ge2,0)
		local ge3=Effect.CreateEffect(c)
		ge3:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge3:SetCode(EVENT_BATTLED)
		ge3:SetOperation(c101010402.recount2)
	...
	ge4:SetCondition(c101010402.rescon)
	ge4:SetOperation(c101010402.resop)
	ge4:SetValue(0x8773)
	...
	ge5:SetTarget(c101010402.pointchk)
	ge5:SetOperation(c101010402.point)
...
function c101010402.ipoint(e,tp,eg,ep,ev,re,r,rp)
...
function c101010402.recount(e,tp,eg,ep,ev,re,r,rp)
...
function c101010402.recheck(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	local ct=tc:GetFlagEffectLabel(10001000)
	if ct then
...
function c101010402.recount2(e,tp,eg,ep,ev,re,r,rp)
	Duel.RegisterFlagEffect(tp,10001000,RESET_PHASE+PHASE_END,0,1)
end
function c101010402.resfilter(c)
...
function c101010402.rescon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	local g=Duel.GetMatchingGroup(c101010402.resfilter,tp,0xbe,0,nil)
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if ft<=-g:FilterCount(Card.IsLocation,nil,LOCATION_MZONE) then return false end
	return g:GetCount()>0 and Duel.IsExistingMatchingCard(c101010402.refilter,tp,LOCATION_EXTRA,0,1,nil,e,tp)
end
function c101010402.fdfilter(c)
	return c:IsFacedown() or c:IsLocation(LOCATION_HAND)
end
function c101010402.refilter(c,e,tp)
	return c:IsFaceup() and c:IsType(TYPE_PENDULUM)
		and c:IsHasEffect(10001000) and not c:IsForbidden()
end
function c101010402.refilter1(c,e,tp)
	return not c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ) and c101010402.refilter(c,e,tp)
end
function c101010402.refilter2(c,e,tp)
	return c:IsType(TYPE_FUSION+TYPE_SYNCHRO+TYPE_XYZ) and c101010402.refilter(c,e,tp)
end
function c101010402.resop(e,tp,eg,ep,ev,re,r,rp,c,sg,og)
	local g=Duel.GetMatchingGroup(c101010402.resfilter,tp,0xbe,0,nil)
	local fg=g:Filter(c101010402.fdfilter,nil)
	...
	local ft=Duel.GetLocationCount(tp,LOCATION_MZONE)
	if Duel.IsExistingMatchingCard(c101010402.refilter1,tp,LOCATION_EXTRA,0,1,nil,e,tp) and Duel.SelectYesNo(tp,aux.Stringid(101010401,5)) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local rg=Duel.SelectMatchingCard(tp,c101010402.refilter1,tp,LOCATION_EXTRA,0,1,ft,nil,e,tp)
		sg:Merge(rg)
	end
	if (not rg or ft>rg:GetCount()) and Duel.IsExistingMatchingCard(c101010402.refilter2,tp,LOCATION_EXTRA,0,1,nil,e,tp) and (not rg or Duel.SelectYesNo(tp,aux.Stringid(101010401,1))) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_SPSUMMON)
		local pc=Duel.SelectMatchingCard(tp,c101010402.refilter2,tp,LOCATION_EXTRA,0,1,1,nil,e,tp)
		sg:Merge(pc)
	end
	...
	while sc do
		...
		e1:SetLabel(pt1+pt2)
		e1:SetOperation(c101010402.pointop)
		sc:RegisterEffect(e1)
		sc=sg:GetNext()
	end
end
function c101010402.pointop(e,tp,eg,ep,ev,re,r,rp)
	local pt=e:GetLabel()
	local c=e:GetHandler()
...
function c101010402.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	local ct=e:GetLabel()
	local c=e:GetHandler()
	local point=c:GetFlagEffectLabel(10001100)
...
function c101010402.damtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local bc=Duel.GetAttackTarget()
	if chk==0 then return bc~=nil end
	local atk1=c:GetAttack()
	local atk2=bc:GetAttack()
	local ct=Duel.GetMatchingGroupCount(c101010402.filter,tp,LOCATION_MZONE,0,nil)*500
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,1-tp,math.abs(atk1-atk2)+ct)
end
function c101010402.damop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local bc=Duel.GetAttackTarget()
