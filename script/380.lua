...
function c101010267.initial_effect(c)
	...
	e0:SetOperation(c101010267.activate)
	c:RegisterEffect(e0)
	--search
	...
	e1:SetTarget(c101010267.tg)
	e1:SetOperation(c101010267.op)
	c:RegisterEffect(e1)
	--cannot target
	...
	e6:SetTarget(c101010267.val)
	c:RegisterEffect(e6)
end
function c101010267.ecfilter(c,f)
...
function c101010267.activate(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_ATOHAND)
	local g1=Duel.SelectMatchingCard(tp,c101010267.ecfilter,tp,LOCATION_DECK,0,1,1,nil,0)
	local tc1=g1:GetFirst()
	Duel.Hint(HINT_SELECTMSG,1-tp,HINTMSG_ATOHAND)
	local g2=Duel.SelectMatchingCard(1-tp,c101010267.ecfilter,tp,0,LOCATION_DECK,1,1,nil,0)
...
function c101010267.tg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and chkc:IsFaceup() end
	if chk==0 then return Duel.IsExistingMatchingCard(c101010267.ecfilter,tp,LOCATION_DECK+LOCATION_HAND,0,1,nil,1)
	...
	local g=Duel.GetMatchingGroup(c101010267.ecfilter,tp,LOCATION_DECK+LOCATION_HAND,0,nil,1)
	Duel.SetOperationInfo(0,CATEGORY_EQUIP,g,1,0,0)
end
function c101010267.op(e,tp,eg,ep,ev,re,r,rp)
	...
	local g1=Duel.SelectMatchingCard(tp,c101010267.ecfilter,tp,LOCATION_HAND+LOCATION_DECK,0,1,1,nil,1)
	local tc1=g1:GetFirst()
...
function c101010267.reptg(c)
	return c:GetEquipCount()~=0
end
function c101010267.val(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	if chk==0 then return Duel.IsExistingMatchingCard(c101010267.reptg,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,1,nil) and c==Duel.GetFieldCard(tp,LOCATION_SZONE,5) end
	return true
end
