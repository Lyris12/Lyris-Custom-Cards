...
function ref.sfilter(c,e,tp,code)
...
function ref.operation1(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=tg:FilterSelect(tp,ref.filter,1,1,nil)
	local tc=g:GetFirst()
	if tc and Duel.SendtoHand(tc,nil,REASON_EFFECT)~=0 and tc:IsLocation(LOCATION_HAND) and Duel.GetLocationCount(tp,LOCATION_MZONE)>0 then
		...
		local sg=Duel.SelectMatchingCard(tp,ref.sfilter,tp,LOCATION_HAND,0,1,1,nil,e,tp,tc:GetCode())
...
function ref.condition2(e,tp,eg,ep,ev,re,r,rp)
	...
	if not tg or not tg:IsExists(ref.cfilter,1,nil) then return false end
