...
--special summon
	...
	e3:SetCountLimit(1,101010002)
	e3:SetCondition(c101010002.dtcon)
	e3:SetTarget(c101010002.sptg)
	e3:SetOperation(c101010002.spop)
	c:RegisterEffect(e3)
end
function c101010002.spfilter(c)
	return c:IsFaceup() and c:IsSetCard(0xeeb)
end
function c101010002.spcon(e,c)
	if c==nil then return true end
	return Duel.GetFieldGroupCount(c:GetControler(),LOCATION_MZONE,0)==1
		and Duel.IsExistingMatchingCard(c101010002.spfilter,c:GetControler(),LOCATION_MZONE,0,1,nil)
		and Duel.GetLocationCount(c:GetControler(),LOCATION_MZONE)>0
end
function c101010002.dtcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsReason(REASON_COST) and re:IsHasType(0x7e0) and re:IsActiveType(TYPE_MONSTER)
		and c:IsPreviousLocation(LOCATION_OVERLAY) and re:GetHandler():IsSetCard(0xeeb)
end
function c101010002.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
