...
function ref.filter1(c,e,tp)
	...
	return rk>1 and c:IsFaceup()
		and Duel.IsExistingMatchingCard(ref.filter2,tp,LOCATION_EXTRA,0,1,nil,rk,at,e,tp)
		and not Duel.IsExistingMatchingCard(ref.filter3,tp,LOCATION_MZONE,0,1,nil,rk)
end
function ref.filter2(c,rk,at,e,tp)
...
function ref.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,ref.filter2,tp,LOCATION_EXTRA,0,1,1,nil,tc:GetRank(),tc:GetAttribute(),e,tp)
	local sc=g:GetFirst()
	if sc then
