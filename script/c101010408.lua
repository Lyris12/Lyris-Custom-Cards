...
function c101010408.initial_effect(c)
	...
	c:RegisterEffect(e2)
	local e6=e2:Clone()
	e6:SetDescription(aux.Stringid(101010408,1))
	e6:SetLabel(2)
	c:RegisterEffect(e6)
	--control
	...
	e4:SetTarget(c101010408.target)
	e4:SetOperation(c101010408.operation)
	c:RegisterEffect(e4)
	--destroy
...
function c101010408.bfilter(c,opt)
	if not c:IsAbleToRemoveAsCost() then return false end
	if opt==1 then
		return c:IsSetCard(0x9008) and c:IsType(TYPE_MONSTER) and Duel.IsExistingMatchingCard(c101010408.dsfilter,tp,LOCATION_HAND,0,1,nil)
	elseif opt==2 then
		return c:IsSetCard(0xc008) and c:IsType(TYPE_MONSTER)
	else return false
	end
end
function c101010408.dsfilter(c)
...
function c101010408.actcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010408.bfilter,tp,LOCATION_GRAVE,0,1,nil,e:GetLabel()) end
	local g=Duel.SelectMatchingCard(tp,c101010408.bfilter,tp,LOCATION_GRAVE,0,1,1,nil,e:GetLabel())
	if e:GetLabel()==1 then
		Duel.DiscardHand(tp,c101010408.dsfilter,1,1,REASON_COST,nil)
	end
...
function c101010408.cfilter(c)
	...
	return c:IsReleasableByEffect() and ((c:IsLocation(LOCATION_MZONE) and Duel.CheckReleaseGroup(tp,c101010408.spfilter,2,c,s)) or (c:IsCode(101010187) and c:IsHasEffect(101010187)) or (c:GetOriginalCode()==101010142 and c:GetFlagEffect(101010142)~=0))
end
function c101010408.spfilter(c,s)
	return c:IsSetCard(s)
end
function c101010408.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010408.cfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,1,nil) end
	local g1=Duel.SelectMatchingCard(tp,c101010408.cfilter,tp,LOCATION_MZONE+LOCATION_HAND,0,1,1,nil)
	local tc=g1:GetFirst()
	local s=0xc008
	if tc:IsSetCard(0x9008) then s=0x9008 end
	if (tc:IsCode(101010187) and tc:IsHasEffect(101010187)) then Duel.Release(tc,REASON_COST) return end
	local g2=Duel.SelectReleaseGroup(tp,c101010408.spfilter,2,2,tc,s)
	g1:Merge(g2)
	Duel.HintSelection(g1)
	Duel.Release(g1,REASON_COST)
end
function c101010408.sptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.GetLocationCount(tp,LOCATION_MZONE)>-3
		and e:GetHandler():IsCanBeSpecialSummoned(e,SUMMON_TYPE_RITUAL,tp,false,true) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,e:GetHandler(),1,0,0)
end
function c101010408.spop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
...
function c101010408.target(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:GetLocation()==LOCATION_MZONE and chkc:GetControler()~=tp and c101010408.filter(chkc) end
...
function c101010408.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(c101010408.dfilter,tp,LOCATION_MZONE,0,1,e:GetHandler()) end
	local g=Duel.GetMatchingGroup(c101010408.dfilter,tp,LOCATION_MZONE,0,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
end
function c101010408.desop(e,tp,eg,ep,ev,re,r,rp)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectMatchingCard(tp,c101010408.dfilter,tp,LOCATION_MZONE,0,1,1,e:GetHandler())
