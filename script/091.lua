...
function c101010170.initial_effect(c)
	...
	e2:SetCondition(c101010170.actcon)
	e2:SetTarget(c101010170.acttg)
	e2:SetOperation(c101010170.act)
	c:RegisterEffect(e2)
	local e3=e2:Clone()
	...
	e5:SetCountLimit(1)
	e5:SetTarget(c101010170.sptg)
	e5:SetOperation(c101010170.spop)
	e5:SetLabelObject(g)
	c:RegisterEffect(e5)
	--spsummon
	if not c101010170.global_check then
		c101010170.global_check=true
		local e4=Effect.CreateEffect(c)
		e4:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		e4:SetCode(EVENT_TO_GRAVE)
		e4:SetOperation(c101010170.operation)
		Duel.RegisterEffect(e4,0)
		e4:SetLabelObject(g)
	end
end
function c101010170.dfilter(c,tp)
	return c:IsReason(REASON_DESTROY)
		and c:IsSetCard(0x9008) and c:GetPreviousControler()==tp
end
function c101010170.operation(e,tp,eg,ep,ev,re,r,rp)
	local g=eg:Filter(c101010170.dfilter,nil,tp)
...
function c101010170.act(e,tp,eg,ep,ev,re,r,rp)
	...
	if c:IsRelateToEffect(e) then
		...
		c101010170.after(e,tp)
	end
end
function c101010170.after(e,tp)
	local g=Duel.GetMatchingGroup(c101010170.actfilter,tp,LOCATION_DECK,0,nil)
