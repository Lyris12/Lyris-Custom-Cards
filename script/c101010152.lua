...
function ref.start(c)
	...
	e6:SetCondition(ref.tgcon)
	e6:SetTarget(ref.destg)
	e6:SetOperation(ref.desop)
	c:RegisterEffect(e6)
end
...
function ref.destg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	local g=Duel.GetMatchingGroup(ref.filter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,nil)
...
function ref.desop(e,tp,eg,ep,ev,re,r,rp)
	...
	local g=Duel.SelectMatchingCard(tp,ref.filter,tp,LOCATION_ONFIELD+LOCATION_HAND,0,1,1,nil)
