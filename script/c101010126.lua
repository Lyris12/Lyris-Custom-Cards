...
--Activate
	...
	tag:SetTarget(c101010126.tagtg)
	tag:SetValue(c101010126.tagval)
	tag:SetOperation(c101010126.tagop)
	c:RegisterEffect(tag)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e1:SetRange(LOCATION_FZONE)
	e1:SetCode(EVENT_CONTROL_CHANGED)
	e1:SetCondition(c101010126.tagcon2)
	...
	nr1:SetCondition(c101010126.nrcon)
	nr1:SetOperation(c101010126.nrop)
...
function c101010126.tfilter(c)
	return (c:IsType(TYPE_FUSION) or c:IsType(TYPE_SYNCHRO) or c:IsType(TYPE_XYZ)) and c:GetTextAttack()~=-2
end
function c101010126.tagtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	if chk==0 then return re~=e and eg:IsExists(c101010126.filter2,1,nil,e) and Duel.GetLocationCount(tc:GetOwner(),LOCATION_MZONE)>-1
		and Duel.IsExistingMatchingCard(c101010126.tfilter,tc:GetOwner(),LOCATION_EXTRA,0,1,nil,e)
	end
...
function c101010126.tagval(e,c)
	return c101010126.filter2(c,e)
end
function c101010126.tagtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
...
function c101010126.lvfilter(c,lv)
	return c:GetLevel()==lv
end
function c101010126.tagop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=e:GetLabelObject()
	local exg=Duel.GetMatchingGroup(c101010126.tfilter,tc:GetOwner(),LOCATION_EXTRA,0,tc,e)
	if Duel.GetLocationCount(tc:GetOwner(),LOCATION_MZONE)>0 and exg:GetCount()>0 then
		...
		if Duel.SpecialSummon(ex,st,tc:GetOwner(),tc:GetOwner(),false,true,POS_FACEUP_ATTACK) and st==SUMMON_TYPE_XYZ then
			...
			local oc2=g:FilterSelect(tc:GetOwner(),c101010126.lvfilter,1,1,oc:GetFirst(),lv)
			oc:Merge(oc2)
			Duel.Overlay(ex,oc)
		end
...
function c101010126.tagop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=eg:GetFirst()
	if eg:GetCount()>1 then
...
function c101010126.nrop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetCurrentChain()==0 then
		Duel.SetChainLimitTillChainEnd(c101010126.nrfilter)
		else
		e:GetHandler():RegisterFlagEffect(101010126,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
	end
end
function c101010126.nrop2(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(101010126)~=0 then
		Duel.SetChainLimitTillChainEnd(c101010126.nrfilter)
	end
	e:GetHandler():ResetFlagEffect(101010126)
end
function c101010126.nrfilter(e,tp,rp)
	return rp~=tp
end
function c101010126.filter1(c,e)
	return c:IsCanBeFusionMaterial() and c:IsAbleToRemove() and not c:IsImmuneToEffect(e)
end
function c101010126.filter3(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and (not f or f(c))
	and c:CheckFusionMaterial(m,nil,chkf)
end
function c101010126.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
		local mg1=Duel.GetMatchingGroup(c101010126.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
		local res=Duel.IsExistingMatchingCard(c101010126.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101010126.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c101010126.fusop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local mg1=Duel.GetMatchingGroup(c101010126.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
	local sg1=Duel.GetMatchingGroup(c101010126.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101010126.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
...
function c101010126.filter(c)
	return c:GetCode()~=101010126
end
function c101010126.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)  
	if Duel.GetMatchingGroup(c101010126.filter,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()>39
		and tc==nil then
		Duel.MoveToField(e:GetHandler(),tp,tp,LOCATION_SZONE,POS_FACEUP,true)
		if tc2==nil then
			local token=Duel.CreateToken(tp,101010126,nil,nil,nil,nil,nil,nil)  
