...
--Activate
	...
	e0:SetValue(c101010225.monval)
	c:RegisterEffect(e0)
end
function c101010225.monval(e,c)
...
function c101010225.filter1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function c101010225.filter2(c,e,tp,m,gc,chkf)
...
function c101010225.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		...
		return Duel.IsExistingMatchingCard(c101010225.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg,e:GetHandler(),chkf)
	end
...
function c101010225.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	local mg=Duel.GetMatchingGroup(c101010225.filter1,tp,LOCATION_MZONE,0,nil,e)
	local sg=Duel.GetMatchingGroup(c101010225.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg,e:GetHandler(),chkf)
