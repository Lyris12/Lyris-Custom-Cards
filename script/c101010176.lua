...
function ref.mttg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	...
	if Duel.GetFlagEffect(tp,id)==0 and Duel.IsExistingMatchingCard(ref.matfilter,tp,LOCATION_GRAVE,0,1,nil)
		and Duel.IsExistingTarget(ref.xyzfilter,tp,LOCATION_MZONE,0,1,nil)
		and Duel.IsExistingMatchingCard(Card.IsSetCard,tp,LOCATION_GRAVE,0,2,nil,0xeeb) and Duel.SelectYesNo(tp,aux.Stringid(id,0)) then
...
function ref.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return Duel.IsExistingMatchingCard(ref.matfilter,tp,LOCATION_GRAVE,0,1,nil) and Duel.GetFlagEffect(tp,id)==0 end
