...
c:EnableReviveLimit()
	...
	e2:SetCondition(function(e) return Duel.IsExistingMatchingCard(c101010284.cfilter,e:GetHandler():GetControler(),LOCATION_HAND,0,2,nil,e) end)
	...
	e3:SetCondition(function(e) return Duel.IsExistingMatchingCard(c101010284.radiant,tp,LOCATION_HAND,0,1,nil) end)
...
function c101010284.spcost(e,tp,eg,ep,ev,re,r,rp,chk)
	...
	local t=g:GetCount()
	while t/2-math.floor(t/2)==0.5 do g=Duel.SelectMatchingCard(tp,c101010284.cfilter,tp,LOCATION_HAND,0,2,62,nil,e) t=g:GetCount() end
	local tc=g:GetFirst()
	while tc do
...
function c101010284.spfilter(c,e)
	return c:IsPublic() and c:GetFlagEffect(101010284)~=0
end
function c101010284.spop(e,tp,eg,ep,ev,re,r,rp)
	if not Duel.IsExistingMatchingCard(c101010284.radiant,tp,LOCATION_HAND,0,1,nil) and e:GetHandler():IsDisabled() then Duel.NegateEffect(0) return end
	...
	for i=1,ht do
		...
		if ft<=0 or cg:GetCount()>0 and not Duel.SelectYesNo(tp,aux.Stringid(101010284,0)) then break end
...
function c101010284.light(e,tp,ct,ft)
	...
	e1:SetOperation(c101010284.chop)
...
function c101010284.chop(e,tp,eg,ep,ev,re,r,rp)
