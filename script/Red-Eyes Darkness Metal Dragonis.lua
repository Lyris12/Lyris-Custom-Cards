...
function c101010280.initial_effect(c)
	...
	e9:SetCondition(c101010280.con)
	e9:SetValue(c101010280.val)
	c:RegisterEffect(e9)
	--damage
	...
	e8:SetCondition(c101010280.con)
	e8:SetCost(c101010280.damcost)
	e8:SetTarget(c101010280.damtg)
	e8:SetOperation(c101010280.damop)
	c:RegisterEffect(e8)
	if not c101010280.global_check then
		c101010280.global_check=true
		local ge2=Effect.CreateEffect(c)
		ge2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_CONTINUOUS)
		ge2:SetCode(EVENT_ADJUST)
		ge2:SetCountLimit(1)
		ge2:SetProperty(EFFECT_FLAG_NO_TURN_RESET)
		ge2:SetOperation(c101010280.chk)
		Duel.RegisterEffect(ge2,0)
	end
end
c101010280.evolute=true
c101010280.material1=function(mc) return mc:IsCode(101010279) and mc:IsFaceup() end
c101010280.material2=function(mc) return mc:IsRace(RACE_DRAGON) and mc:IsFaceup() end
function c101010280.chk(e,tp,eg,ep,ev,re,r,rp)
	Duel.CreateToken(tp,388)
	Duel.CreateToken(1-tp,388)
end
function c101010280.val(e,c)
...
function c101010280.damcost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010280.filter(c,e,tp)
	return c:IsRace(RACE_DRAGON) and c:IsDestructable()
end
function c101010280.damtg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and c101010280.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c101010280.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,e:GetHandler()) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_DESTROY)
	local g=Duel.SelectTarget(tp,c101010280.filter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,e:GetHandler())
	Duel.SetOperationInfo(0,CATEGORY_DESTROY,g,1,0,0)
	Duel.SetOperationInfo(0,CATEGORY_DAMAGE,nil,0,PLAYER_ALL,0)
end
function c101010280.damop(e,tp,eg,ep,ev,re,r,rp)
