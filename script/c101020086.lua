...
function c101020086.initial_effect(c)
	...
	e1:SetCountLimit(1)
	e1:SetCost(c101020086.bancost)
	e1:SetTarget(c101020086.bantg)
	e1:SetOperation(c101020086.banop)
	c:RegisterEffect(e1)
	--end turn/order and banish deck
	local e2=Effect.CreateEffect(c)
	e2:SetDescription(aux.Stringid(101020086,0))
	e2:SetCategory(CATEGORY_REMOVE)
	e2:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e2:SetCode(EVENT_PHASE+PHASE_END)
	e2:SetRange(LOCATION_MZONE)
	e2:SetCountLimit(1)
	e2:SetCondition(c101020086.etcon)
	e2:SetTarget(c101020086.ettg)
	e2:SetOperation(c101020086.etop)
	c:RegisterEffect(e2)
	--selfbanish
	...
	e3:SetOperation(c101020086.sbop)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	e4:SetCode(EVENT_ADJUST)
	c:RegisterEffect(e4)
end
function c101020086.bancost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return e:GetHandler():CheckRemoveOverlayCard(tp,1,REASON_COST) end
	e:GetHandler():RemoveOverlayCard(tp,1,1,REASON_COST)
end
function c101020086.bantg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
...
function c101020086.banop(e,tp,eg,ep,ev,re,r,rp)
	...
	if tc and tc:IsRelateToEffect(e) and Duel.Remove(tc,POS_FACEUP,REASON_EFFECT)>0 and c:IsRelateToEffect(e) then
		...
		e2:SetTarget(c101020086.tg)
		e2:SetReset(RESET_EVENT+0x1ff0000)
		e2:SetLabel(code)
		c:RegisterEffect(e2)
	end
end
function c101020086.tg(e,c)
	return c:IsCode(e:GetLabel()) and (not c:IsOnField() or c:GetRealFieldID()>e:GetFieldID())
end
function c101020086.etcon(e,tp,eg,ep,ev,re,r,rp)
	return Duel.GetTurnPlayer()==tp
end
function c101020086.ettg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return true end
	Duel.SetOperationInfo(0,CATEGORY_REMOVE,nil,1,0,LOCATION_DECK)
end
function c101020086.etop(e,tp,eg,ep,ev,re,r,rp)
...
function c101020086.sbop(e,tp,eg,ep,ev,re,r,rp)
