...
function ref.start(c)
	...
	e1:SetCountLimit(1)
	e1:SetCost(ref.bancost)
	e1:SetTarget(ref.bantg)
	e1:SetOperation(ref.banop)
	c:RegisterEffect(e1)
	...
	e2:SetCountLimit(1)
	e2:SetCondition(ref.etcon)
	e2:SetTarget(ref.ettg)
	e2:SetOperation(ref.etop)
	c:RegisterEffect(e2)
	--selfbanish
	...
	e3:SetOperation(ref.sbop)
	c:RegisterEffect(e3)
	local e4=e3:Clone()
	e4:SetCode(EVENT_ADJUST)
	c:RegisterEffect(e4)
end
function ref.bancost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function ref.sbop(e,tp,eg,ep,ev,re,r,rp)
