...
function ref.spfilter(c,e,tp,rg)
...
function ref.cfilter(c,lv)
	return c:GetLevel()<lv and c:IsType(TYPE_TUNER) and c:IsAbleToGrave() and c:IsCanBeSynchroMaterial()
end
function ref.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local rg=Duel.GetMatchingGroup(ref.rmfilter,tp,LOCATION_MZONE,0,nil)
		local g=Duel.GetFieldGroup(tp,LOCATION_DECK,0):GetMaxGroup(Card.GetLevel)
		if not rg:IsExists(ref.cfilter,1,nil,g:GetFirst():GetLevel()) then return false end
		return Duel.IsExistingMatchingCard(ref.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,rg)
	end
	local rg=Duel.GetMatchingGroup(ref.rmfilter,tp,LOCATION_MZONE,0,nil)
	local g=Duel.GetFieldGroup(tp,LOCATION_DECK,0):GetMaxGroup(Card.GetLevel)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tuc=rg:FilterSelect(tp,ref.cfilter,1,1,nil,g:GetFirst():GetLevel()):GetFirst()
...
function ref.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=sg:FilterSelect(tp,ref.spfilter,1,1,nil,e,tp,rg)
