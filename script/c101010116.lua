...
function c101010205.initial_effect(c)
	...
	e1:SetTarget(c101010205.target)
	e1:SetOperation(c101010205.activate)
	c:RegisterEffect(e1)
end
c101010205.sea_scout_list=true
function c101010205.spfilter(c,e,tp,rg)
...
function c101010205.rmfilter(c)
	return c:GetLevel()>0 and c:IsAbleToGrave()
end
function c101010205.cfilter(c,lv)
	return c:GetLevel()<lv and c:IsType(TYPE_TUNER) and c:IsAbleToGrave() and c:IsCanBeSynchroMaterial()
end
function c101010205.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local rg=Duel.GetMatchingGroup(c101010205.rmfilter,tp,LOCATION_MZONE,0,nil)
		local g=Duel.GetFieldGroup(tp,LOCATION_DECK,0):GetMaxGroup(Card.GetLevel)
		if not rg:IsExists(c101010205.cfilter,1,nil,g:GetFirst():GetLevel()) then return false end
		return Duel.IsExistingMatchingCard(c101010205.spfilter,tp,LOCATION_DECK,0,1,nil,e,tp,rg)
	end
	local rg=Duel.GetMatchingGroup(c101010205.rmfilter,tp,LOCATION_MZONE,0,nil)
	local g=Duel.GetFieldGroup(tp,LOCATION_DECK,0):GetMaxGroup(Card.GetLevel)
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TOGRAVE)
	local tuc=rg:FilterSelect(tp,c101010205.cfilter,1,1,nil,g:GetFirst():GetLevel()):GetFirst()
	Duel.SetTargetCard(tuc)
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,1,tp,LOCATION_DECK)
end
function c101010205.activate(e,tp,eg,ep,ev,re,r,rp)
	...
	local sc=sg:FilterSelect(tp,c101010205.spfilter,1,1,nil,e,tp,rg)
