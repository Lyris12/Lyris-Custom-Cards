--ファイヤーストローム・ツイスター
function c101010571.initial_effect(c)
	local e1=Effect.CreateEffect(c)
	e1:SetCategory(CATEGORY_DESTROY)
	e1:SetType(EFFECT_TYPE_ACTIVATE)
	e1:SetCode(EVENT_FREE_CHAIN)
	e1:SetTarget(c101010571.target)
	e1:SetOperation(c101010571.activate)
	c:RegisterEffect(e1)
end
function c101010571.filter(c)
	return c:IsFaceup() and c:IsSetCard(0xa88)
end
function c101010571.dfilter(c)
	return c:IsFacedown() or not (c:IsSetCard(0xa88) and c:IsType(TYPE_MONSTER))
end
function c101010571.target(e,tp,eg,ep,ev,re,r,rp,chk)
	local c=e:GetHandler()
	local g=Duel.GetMatchingGroup(c101010571.filter,tp,LOCATION_MZONE,0,nil)
	...
	if b1 and b2 then
		local opt=Duel.SelectOption(tp,aux.Stringid(101010571,1),aux.Stringid(101010571,2))
		e:SetLabel(opt)
	elseif b1 then
		Duel.SelectOption(tp,aux.Stringid(101010571,1))
		e:SetLabel(0)
	else
		Duel.SelectOption(tp,aux.Stringid(101010571,2))
		e:SetLabel(1)
	end
	...
	if e:GetLabel()==0 then
		...
		local des=dg:Filter(c101010571.dfilter,nil)
		Duel.SetOperationInfo(0,CATEGORY_DESTROY,des,des:GetCount(),0,0)
	end
end
function c101010571.activate(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local ct=e:GetLabel()
	local dg=Duel.GetMatchingGroup(Card.IsDestructable,tp,LOCATION_ONFIELD,LOCATION_ONFIELD,c)
