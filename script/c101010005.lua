...
--You can target 1 "Earth Enforcer" Xyz Monster you control; discard this card from your hand, then attach 1 "Earth Enforcer" monster from your Deck to that Xyz Monster as an Xyz Material.
	...
	e3:SetCondition(c101010005.dtcon)
	...
	e3:SetTarget(c101010005.thtg)
	e3:SetOperation(c101010005.thop)
	c:RegisterEffect(e3)
end
function c101010005.xyzfilter(c)
	return c:IsFaceup() and c:IsType(TYPE_XYZ) and c:IsSetCard(0xeeb)
end
function c101010005.matfilter(c)
	return c:IsSetCard(0xeeb) and c:IsType(TYPE_MONSTER)
end
function c101010005.mattg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsControler(tp) and chkc:IsLocation(LOCATION_MZONE) and c101010005.xyzfilter(chkc) end
	if chk==0 then return e:GetHandler():IsDiscardable(REASON_EFFECT) and Duel.IsExistingTarget(c101010005.xyzfilter,tp,LOCATION_MZONE,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_FACEUP)
	Duel.SelectTarget(tp,c101010005.xyzfilter,tp,LOCATION_MZONE,0,1,1,nil)
end
function c101010005.matop(e,tp,eg,ep,ev,re,r,rp)
	if not e:GetHandler():IsRelateToEffect(e) then return end
	Duel.SendtoGrave(e:GetHandler(),REASON_EFFECT+REASON_DISCARD)
	local tc=Duel.GetFirstTarget()
	if tc:IsFaceup() and tc:IsRelateToEffect(e) and not tc:IsImmuneToEffect(e) then
		Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_XMATERIAL)
		local g=Duel.SelectMatchingCard(tp,c101010005.matfilter,tp,LOCATION_DECK,0,1,1,nil)
...
function c101010005.dtcon(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	return c:IsReason(REASON_COST) and re:IsHasType(0x7e0) and re:IsActiveType(TYPE_MONSTER)
		and c:IsPreviousLocation(LOCATION_OVERLAY) and re:GetHandler():IsSetCard(0xeeb)
end
function c101010005.thtg(e,tp,eg,ep,ev,re,r,rp,chk)
