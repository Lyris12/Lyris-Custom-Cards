...
--additional effects
	...
	ae4:SetCondition(c101010213.rmcon)
	ae4:SetOperation(c101010213.rmop)
	...
	ae1:SetCondition(c101010213.tgcon)
	ae1:SetValue(1)
	...
	ae2:SetCondition(c101010213.atkucon)
	ae2:SetOperation(c101010213.atkuop)
	...
	e1:SetCondition(c101010213.spcon)
	e1:SetOperation(c101010213.spop)
...
function c101010213.spfilter(c,v,y,z)
	local atk=math.abs(c:GetAttack()-c:GetDefense())
	return c:GetRank()==v and atk>=y and atk<=z and c:IsAbleToRemoveAsCost()
end
function c101010213.spcon(e,c)
	if c==nil then return true end
	local tp=c:GetControler()
	return Duel.GetLocationCount(tp,LOCATION_MZONE)>-1 and Duel.IsExistingMatchingCard(c101010213.spfilter,tp,LOCATION_MZONE,0,1,nil,5,250,750)
end
function c101010213.spop(e,tp,eg,ep,ev,re,r,rp,c)
	local g=Duel.SelectMatchingCard(tp,c101010213.spfilter,tp,LOCATION_MZONE,0,1,1,nil,5,250,750)
...
function c101010213.rmcon(e,tp,eg,ep,ev,re,r,rp)
...
function c101010213.rmop(e,tp,eg,ep,ev,re,r,rp)
	local p=Duel.GetChainInfo(0,CHAININFO_TRIGGERING_CONTROLER)
	if p==tp then return end
	Duel.Remove(re:GetHandler(),POS_FACEUP,REASON_EFFECT)
end
function c101010213.tgcon(e)
	return bit.band(e:GetHandler():GetSummonType(),SUMMON_TYPE_XYZ)==SUMMON_TYPE_XYZ
end
function c101010213.atkucon(e)
	return bit.band(e:GetHandler():GetSummonType(),SUMMON_TYPE_XYZ)==0
end
function c101010213.atfilter(c)
	return c:IsRace(RACE_MACHINE) and not c:IsReason(REASON_MATERIAL)
end
function c101010213.atkuop(e,tp,eg,ep,ev,re,r,rp)
	local ct=Duel.GetMatchingGroupCount(c101010213.atfilter,tp,LOCATION_REMOVED,LOCATION_REMOVED,nil)
	local c=e:GetHandler()
