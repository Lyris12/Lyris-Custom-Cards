...
function c101010159.initial_effect(c)
	...
	e1:SetTarget(c101010159.target)
	e1:SetOperation(c101010159.activate)
	c:RegisterEffect(e1)
end
function c101010159.chk(e,tp)
	...
	for i=0,4 do
		for j=0,4 do
			...
			if c1 and c2 and c1:IsCanBeFusionMaterial() and c2:IsCanBeFusionMaterial() then
				if Duel.IsExistingMatchingCard(c101010159.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,g,nil,chkf) 
	...
	return c1 and c2 and c3 and c4 and c5 and c1:IsCanBeFusionMaterial() and c2:IsCanBeFusionMaterial() and c3:IsCanBeFusionMaterial() 
		...
		and Duel.IsExistingMatchingCard(c101010159.filter2,tp,LOCATION_EXTRA,0,1,nil,e,tp,g,nil,chkf) 
		and Duel.IsExistingMatchingCard(Card.IsXyzSummonable,tp,LOCATION_EXTRA,0,1,nil,g,0,5)
end
function c101010159.filter1(c,e)
	return c:IsCanBeFusionMaterial() and not c:IsImmuneToEffect(e)
end
function c101010159.filter2(c,e,tp,m,f)
	return c:IsType(TYPE_FUSION) and (not f or f(c))
		and c:IsCanBeSpecialSummoned(e,SUMMON_TYPE_FUSION,tp,false,false) and c:CheckFusionMaterial(m,nil)
end
function c101010159.target(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return c101010159.chk(e,tp) end
	Duel.SetOperationInfo(0,CATEGORY_SPECIAL_SUMMON,nil,2,tp,LOCATION_EXTRA)
end
function c101010159.activate(e,tp,eg,ep,ev,re,r,rp)
	if not c101010159.chk(e,tp) then return end
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	local mg1=Duel.GetMatchingGroup(c101010159.filter1,tp,LOCATION_MZONE,0,nil,e)
	local sg1=Duel.GetMatchingGroup(c101010159.filter2,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
