...
function ref.distg(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsLocation(LOCATION_MZONE) and ref.disfilter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(ref.disfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_TARGET)
	local g=Duel.SelectTarget(tp,ref.disfilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,1,nil)
