...
function c101010225.initial_effect(c)
	...
	e0:SetCondition(c101010225.con)
	e0:SetCost(c101010225.cost)
	e0:SetTarget(c101010225.tg)
	e0:SetOperation(c101010225.activate)
	c:RegisterEffect(e0)
		--destroy replace
		...
		e1:SetCondition(c101010225.repcon)
		e1:SetTarget(c101010225.reptg)
		e1:SetValue(c101010225.repval)
		c:RegisterEffect(e1)
end
function c101010225.repcon(e,tp,eg,ep,ev,re,r,rp)
	return e:GetHandler():IsFacedown()
end
function c101010225.repfilter(c,tp)
	return c:IsFaceup() and c:IsControler(tp) and c:IsLocation(LOCATION_MZONE) and c:IsAttribute(ATTRIBUTE_WATER)
end
function c101010225.reptg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then return eg:IsExists(c101010225.repfilter,1,nil,tp) end
	if e:GetHandler():IsAbleToGrave() and Duel.SelectYesNo(tp,aux.Stringid(101010225,0)) then
		Duel.SendtoGrave(e:GetHandler(),REASON_EFFECT)
		return true
	else return false end
end
function c101010225.repval(e,c)
	return c101010225.repfilter(c,e:GetHandlerPlayer())
end
function c101010225.con(e,tp,eg,ep,ev,re,r,rp)
	local rc=re:GetHandler()
	return re:IsActiveType(TYPE_MONSTER) and not rc:IsAttribute(ATTRIBUTE_WATER) and Duel.IsChainNegatable(ev)
end
function c101010225.cost(e,tp,eg,ep,ev,re,r,rp,chk)
...
function c101010225.tg(e,tp,eg,ep,ev,re,r,rp,chk)
