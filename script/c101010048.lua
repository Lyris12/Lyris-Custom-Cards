...
aux.EnablePendulumAttribute(c,false)
	...
	e0:SetCondition(c101010048.xcon)
	e0:SetTarget(c101010048.xtg)
	...
	e2:SetLabel(1)
	e2:SetCondition(c101010048.con)
	...
	e6:SetCondition(c101010048.atcon)
	c:RegisterEffect(e6)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD)
	e1:SetCode(EFFECT_SET_POSITION)
	e1:SetRange(LOCATION_MZONE)
	e1:SetTarget(function(e,c) return c:IsPosition(POS_FACEUP_DEFENSE) and c:IsAttribute(ATTRIBUTE_FIRE+ATTRIBUTE_WATER+ATTRIBUTE_WIND) end)
	...
	e5:SetCondition(c101010048.ccon)
	e5:SetValue(aux.imval1)
	c:RegisterEffect(e5)
end
c101010048.pendulum_level=5
...
function c101010048.xcon(e,c,og)
	...
	local g=Duel.GetMatchingGroup(c101010048.xfilter,tp,LOCATION_MZONE,0,nil,c)
...
function c101010048.con(e,tp,eg,ep,ev,re,r,rp)
	if e:GetLabel()==1 and not Duel.IsExistingMatchingCard(c101010048.efilter,tp,LOCATION_MZONE,0,1,e:GetHandler(),1) then return false end
	if e:GetLabel()==2 and not Duel.IsExistingMatchingCard(c101010048.efilter,tp,LOCATION_MZONE,LOCATION_MZONE,1,nil,0) then return false end
	local g=e:GetHandler():GetOverlayGroup()
	return g:GetCount()>0 and not g:IsExists(c101010048.cfilter,1,nil)
end
function c101010048.filter(c)
	return c:IsAttribute(ATTRIBUTE_FIRE+ATTRIBUTE_WATER+ATTRIBUTE_WIND) and c:IsAttackable()
end
function c101010048.atcon(e)
	return Duel.IsExistingMatchingCard(c101010048.filter,e:GetHandlerPlayer(),LOCATION_MZONE,LOCATION_MZONE,1,nil)
end
function c101010048.ccon(e)
