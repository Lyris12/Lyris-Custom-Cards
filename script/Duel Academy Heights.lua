...
function c101010004.initial_effect(c)
	...
	fus:SetTarget(c101010004.fustg)
	fus:SetOperation(c101010004.fusop)
	...
	tag:SetTarget(c101010004.tagtg)
	tag:SetValue(c101010004.tagval)
	tag:SetOperation(c101010004.tagop)
	c:RegisterEffect(tag)
	local e1=Effect.CreateEffect(c)
	e1:SetType(EFFECT_TYPE_FIELD+EFFECT_TYPE_TRIGGER_F)
	e1:SetRange(LOCATION_FZONE)
	e1:SetCode(EVENT_CONTROL_CHANGED)
	e1:SetCondition(c101010004.tagcon2)
	e1:SetTarget(c101010004.tagtg2)
	e1:SetOperation(c101010004.tagop2)
	c:RegisterEffect(e1)
	...
	nr1:SetCondition(c101010004.nrcon)
	nr1:SetOperation(c101010004.nrop)
	...
	nr2:SetOperation(c101010004.nrop2)
	c:RegisterEffect(nr2)
end
-- function c101010004.tagcon(e,tp,eg,ep,ev,re,r,rp)
	-- local tc=eg:GetFirst()
	-- return tc:GetLocation()~=0x40 and (tc:IsType(TYPE_FUSION) or tc:IsType(TYPE_SYNCHRO) or tc:IsType(TYPE_XYZ))
-- end
function c101010004.tagcon2(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	return tc:IsType(TYPE_FUSION) or tc:IsType(TYPE_SYNCHRO) or tc:IsType(TYPE_XYZ)
end
function c101010004.tfilter(c)
	return (c:IsType(TYPE_FUSION) or c:IsType(TYPE_SYNCHRO) or c:IsType(TYPE_XYZ)) and c:GetTextAttack()~=-2
end
function c101010004.tagtg(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
	if chk==0 then return re~=e and eg:IsExists(c101010004.filter2,1,nil,e) and Duel.GetLocationCount(tc:GetOwner(),LOCATION_MZONE)>-1
		and Duel.IsExistingMatchingCard(c101010004.tfilter,tc:GetOwner(),LOCATION_EXTRA,0,1,nil,e)
	end
...
function c101010004.tagval(e,c)
	return c101010004.filter2(c,e)
end
function c101010004.tagtg2(e,tp,eg,ep,ev,re,r,rp,chk)
	local tc=eg:GetFirst()
...
function c101010004.lvfilter(c,lv)
	return c:GetLevel()==lv
end
function c101010004.tagop(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=e:GetLabelObject()
	local exg=Duel.GetMatchingGroup(c101010004.tfilter,tc:GetOwner(),LOCATION_EXTRA,0,tc,e)
	if Duel.GetLocationCount(tc:GetOwner(),LOCATION_MZONE)>0 and exg:GetCount()>0 then
		...
		if Duel.SpecialSummon(ex,st,tc:GetOwner(),tc:GetOwner(),false,true,POS_FACEUP_ATTACK) and st==SUMMON_TYPE_XYZ then
			...
			local oc2=g:FilterSelect(tc:GetOwner(),c101010004.lvfilter,1,1,oc:GetFirst(),lv)
			oc:Merge(oc2)
			Duel.Overlay(ex,oc)
		end
...
function c101010004.tagop2(e,tp,eg,ep,ev,re,r,rp)
	local c=e:GetHandler()
	local tc=eg:GetFirst()
	if eg:GetCount()>1 then
...
function c101010004.filter2(c,e)
...
function c101010004.nrcon(e,tp,eg,ep,ev,re,r,rp)
	local tc=eg:GetFirst()
	return tc:GetPreviousLocation()==LOCATION_EXTRA
end
function c101010004.nrop(e,tp,eg,ep,ev,re,r,rp)
	if Duel.GetCurrentChain()==0 then
		Duel.SetChainLimitTillChainEnd(c101010004.nrfilter)
		else
		e:GetHandler():RegisterFlagEffect(101010004,RESET_EVENT+0x1fe0000+RESET_PHASE+PHASE_END,0,1)
	end
end
function c101010004.nrop2(e,tp,eg,ep,ev,re,r,rp)
	if e:GetHandler():GetFlagEffect(101010004)~=0 then
		Duel.SetChainLimitTillChainEnd(c101010004.nrfilter)
	end
	e:GetHandler():ResetFlagEffect(101010004)
end
function c101010004.nrfilter(e,tp,rp)
	return rp~=tp
end
function c101010004.filter1(c,e)
	return c:IsCanBeFusionMaterial() and c:IsAbleToRemove() and not c:IsImmuneToEffect(e)
end
function c101010004.filter3(c,e,tp,m,f,chkf)
	return c:IsType(TYPE_FUSION) and (not f or f(c))
	and c:CheckFusionMaterial(m,nil,chkf)
end
function c101010004.fustg(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
		local mg1=Duel.GetMatchingGroup(c101010004.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
		local res=Duel.IsExistingMatchingCard(c101010004.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg1,nil,chkf)
		if not res then
			...
			if ce~=nil then
				...
				res=Duel.IsExistingMatchingCard(c101010004.filter3,tp,LOCATION_EXTRA,0,1,nil,e,tp,mg2,mf,chkf)
			end
		end
		return res
	end
...
function c101010004.fusop(e,tp,eg,ep,ev,re,r,rp)
	local chkf=Duel.GetLocationCount(tp,LOCATION_MZONE)>0 and PLAYER_NONE or tp
	if not e:GetHandler():IsRelateToEffect(e) then return end
	local mg1=Duel.GetMatchingGroup(c101010004.filter1,tp,LOCATION_HAND+LOCATION_MZONE,0,nil,e)
	local sg1=Duel.GetMatchingGroup(c101010004.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg1,nil,chkf)
	local mg2=nil
	local sg2=nil
	local ce=Duel.GetChainMaterial(tp)
	if ce~=nil then
		...
		mg2=fgroup(ce,e,tp)
		local mf=ce:GetValue()
		sg2=Duel.GetMatchingGroup(c101010004.filter3,tp,LOCATION_EXTRA,0,nil,e,tp,mg2,mf,chkf)
	end
...
function c101010004.filter(c)
	return c:GetCode()~=101010004
end
function c101010004.op(e,tp,eg,ep,ev,re,r,rp)
	local tc=Duel.GetFieldCard(tp,LOCATION_SZONE,5)
	local tc2=Duel.GetFieldCard(1-tp,LOCATION_SZONE,5)  
	if Duel.GetMatchingGroup(c101010004.filter,tp,LOCATION_HAND+LOCATION_DECK,0,nil):GetCount()>39
		and tc==nil then
