...
function c101010299.cfilter(c,tp,code1,code2)
	local code=c:GetCode()
	return (code==code1 or code==code2) and c:IsAbleToGraveAsCost()--[[ and Duel.IsExistingMatchingCard(c101010299.ffilter,tp,LOCATION_HAND+LOCATION_SZONE,0,1,c,c:GetCode())
end
function c101010299.ffilter(c,code)
	return c:GetCode()~=code and c:IsAbleToGraveAsCost()]]
end
function c101010299.cost(e,tp,eg,ep,ev,re,r,rp,chk)
	if chk==0 then
		local g=Duel.GetMatchingGroup(c101010299.cfilter,tp,LOCATION_HAND+LOCATION_SZONE,0,nil,tp,12670770,86686671)
	...
	local sg=Duel.GetMatchingGroup(c101010299.cfilter,tp,LOCATION_HAND+LOCATION_SZONE,0,nil,tp,12670770,86686671)
