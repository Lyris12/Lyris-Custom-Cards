--FFD－プロミネンス
function c101010566.initial_effect(c)
	--damage conversion
	...
	e2:SetTarget(c101010566.rectg)
	e2:SetOperation(c101010566.recop)
	c:RegisterEffect(e2)
	--fusion summon
	c:EnableReviveLimit()
	aux.AddFusionProcFun2(c,c101010566.ffilter1,c101010566.ffilter2,true)
end
function c101010566.ffilter1(c)
	return c:IsAttribute(ATTRIBUTE_FIRE) or (c:IsHasEffect(101010560) and not c:IsLocation(LOCATION_DECK))
end
function c101010566.ffilter2(c)
	return c:IsRace(RACE_WINDBEAST+RACE_PYRO) or (c:IsHasEffect(101010576) and not c:IsLocation(LOCATION_DECK))
end
function c101010566.rectg(e,tp,eg,ep,ev,re,r,rp,chk)
